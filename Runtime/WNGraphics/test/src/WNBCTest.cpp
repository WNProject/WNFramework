...
#include "WNGraphics/test/inc/bc_test.h"
...
using bc_triangle_test = wn::runtime::graphics::testing::pixel_test<100, 100,
...
TEST_F(bc_triangle_test, basic) {
  ...
  run_test(features, [this, &files](
    ...
    wn::runtime::graphics::image_create_info info = {
        128, 128, wn::runtime::graphics::data_format::bc1_rgb,
    ...
    wn::file_system::file_ptr bc_file =
        files->open_file("assets/gradient_bc.img", res);

    ASSERT_EQ(wn::file_system::result::ok, res);
    const uint8_t* data = bc_file->data();
    data += 32;
