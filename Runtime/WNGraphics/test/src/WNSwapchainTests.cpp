            ...
            for (auto& adapter : device_factory.query_adapters()) {
              ...
              wn::memory::unique_ptr<wn::runtime::window::window> wind =
                  factory.create_window(
                      wn::runtime::window::window_type::system,
              ...
              wn::core::pair<wn::runtime::graphics::surface,
                  ...
                  surface =
                      adapter->make_surface(data->system_allocator, wind.get());
              ASSERT_EQ(
                  wn::runtime::graphics::graphics_error::ok, surface.second);
              wn::runtime::graphics::fence done_present_fence =
                  device->create_fence();
              ...
              auto swapchain = device->create_swapchain(
                  surface.first, create_info, queue.get());
              ...
              struct per_swapchain_image_data {
                ...
                wn::memory::unique_ptr<wn::runtime::graphics::fence>
                    m_ready_fence;
                wn::memory::unique_ptr<wn::runtime::graphics::signal>
                    m_ready_signal;
                wn::runtime::graphics::signal m_present_signal;
              };

              wn::containers::dynamic_array<per_swapchain_image_data>
                  swapchain_data(allocator);
              ...
              // Let's make sure we can render 10 frames successfully on the
              // swapchain
              for (size_t i = 0; i < 10; ++i) {
                ...
                uint32_t idx =
                    swapchain->get_next_backbuffer_index(nullptr, sig.get());
                ...
                queue->enqueue_command_lists({list.get()},
                    {wn::core::make_pair(
                        static_cast<wn::runtime::graphics::pipeline_stages>(
                            wn::runtime::graphics::pipeline_stage::
                                color_attachment_output),
                        frame_data.m_ready_signal.get())},
                    frame_data.m_ready_fence.get(),
                    {&frame_data.m_present_signal});

                frame_data.commmand_lists = wn::core::move(list);
                swapchain->present(
                    queue.get(), &frame_data.m_present_signal, idx);
              }
