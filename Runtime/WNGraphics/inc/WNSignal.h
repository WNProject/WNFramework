...
class signal WN_FINAL : public core::non_copyable {
public:
  signal() = delete;

  WN_FORCE_INLINE signal(signal&& _other) : m_device(_other.m_device) {
...
private:
  ...
  WN_FORCE_INLINE signal(device* _device) : m_data({0}), m_device(_device) {}
