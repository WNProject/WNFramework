...
void vulkan_command_list::enqueue_resource_transition(
  ...
  m_context->vkCmdPipelineBarrier(m_command_buffer,
      ...
      resource_state_to_vulkan_pipeline_stage(_to), 0, 0, 0, 0,
      0, 1, &barrier);
}

void vulkan_command_list::enqueue_texture_upload(const upload_heap& _upload_heap,
...
void vulkan_command_list::enqueue_texture_download(const image& _image,
  ...
  m_context->vkCmdCopyImageToBuffer(m_command_buffer,
    image_res, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, data.buffer, 1, &copy);
}
