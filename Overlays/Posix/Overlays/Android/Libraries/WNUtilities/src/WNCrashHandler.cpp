...
typedef ssize_t (*unwind_backtrace_signal_arch_func)(
    siginfo_t*, void*, const map_info_t*, backtrace_frame_t*, size_t, size_t);
typedef void (*get_backtrace_symbols_func)(
...
struct StackUnwinder {
  StackUnwinder() {
    ...
    for (int i = 0; i < COUNT_OF(signalsToCatch); ++i) {
      ...
      sigAct.sa_flags = SA_RESTART | SA_SIGINFO;  // we want SA_RESTART
                                                  // otherwise all semaphores
                                                  // will fail when we hit this
  ...
  bool InitializeHandler() {
    ...
    acquire_my_map_info_list = reinterpret_cast<acquire_my_map_info_list_func>(
        dlsym(lib, "acquire_my_map_info_list"));
    release_my_map_info_list = reinterpret_cast<release_my_map_info_list_func>(
        dlsym(lib, "release_my_map_info_list"));
    unwind_backtrace_signal_arch =
        reinterpret_cast<unwind_backtrace_signal_arch_func>(
            dlsym(lib, "unwind_backtrace_signal_arch"));
    get_backtrace_symbols = reinterpret_cast<get_backtrace_symbols_func>(
        dlsym(lib, "get_backtrace_symbols"));
    free_backtrace_symbols = reinterpret_cast<free_backtrace_symbols_func>(
        dlsym(lib, "free_backtrace_symbols"));
    return (acquire_my_map_info_list && release_my_map_info_list &&
            unwind_backtrace_signal_arch && get_backtrace_symbols &&
...
void StackUnwinder::error_func(int sig, siginfo_t* info, void* context) {
  ...
  pthread_setspecific(
      g_CrashHandler->mProcessingKey, reinterpret_cast<void*>(1));
  if (g_CrashHandler->InitializeHandler()) {
    int file = open("/sdcard/crash", O_CREAT | O_APPEND | O_WRONLY,
    ...
    ssize_t numFrames = g_CrashHandler->unwind_backtrace_signal_arch(
        info, context, mapInfo, frames, 1, MAX_FRAMES);
    g_CrashHandler->get_backtrace_symbols(frames, numFrames, symbols);
    for (int i = 0; i < numFrames; ++i) {
      for (int j = sizeof(void*) * 2 - 1; j >= 0; --j) {
        unsigned char x =
            0xF & (reinterpret_cast<size_t>(symbols[i].relative_pc) >> (j * 4));
        const char* val = "0";
#define CASE(x)                                                                \
  case 0x##x:                                                                  \
    val = #x;                                                                  \
