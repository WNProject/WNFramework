...
struct vector_traits_sse : vector_traits_generic {
  ...
  static WN_FORCE_INLINE
      typename core::enable_if<core::is_same<type, float>::value>::type
      length_squared(const element_array<float, 2>& _element_array) {
    const __m128 squared = _mm_mul_ps(
        _element_array.m_xmm_values[0], _element_array.m_xmm_values[0]);

    return (_mm_cvtss_f32(_mm_add_ss(
        squared, _mm_shuffle_ps(squared, squared, _MM_SHUFFLE(1, 0, 2, 3)))));
  }

  template <typename type>
  static WN_FORCE_INLINE
      typename core::enable_if<core::is_same<type, float>::value, float>::type
      length_squared(const element_array<float, 4>& _element_array) {
    const __m128 squared = _mm_mul_ps(
        _element_array.m_xmm_values[0], _element_array.m_xmm_values[0]);
    const __m128 partial = _mm_add_ps(
        squared, _mm_shuffle_ps(squared, squared, _MM_SHUFFLE(1, 0, 3, 2)));

    return (_mm_cvtss_f32(_mm_add_ss(
        partial, _mm_shuffle_ps(partial, partial, _MM_SHUFFLE(0, 1, 0, 1)))));
  }
  ...
  template <typename type>
  static WN_FORCE_INLINE
      typename core::enable_if<core::is_same<type, double>::value>::type
      normalize(element_array<float, 4>& _element_array) {
    const __m128 squared = _mm_mul_ps(
        _element_array.m_xmm_values[0], _element_array.m_xmm_values[0]);
    ...
    shuffle =
        _mm_shuffle_ps(length_squared, length_squared, _MM_SHUFFLE(0, 1, 0, 1));
    ...
    _element_array.m_xmm_values[0] =
        _mm_mul_ps(_element_array.m_xmm_values[0], inverse_length);
#else
    const __m128 length = _mm_sqrt_ps(length_squared);

    _element_array.m_xmm_values[0] =
        _mm_div_ps(_element_array.m_xmm_values[0], length);
#endif
  }
  ...
  static WN_FORCE_INLINE
      typename core::enable_if<(dimension > 1 && dimension < 5)>::type
      recipricol(element_array<float, dimension>& _element_array) {
#ifdef __WN_MATH_APPROXIMATIONS_ENABLED
    _element_array.m_xmm_values[0] = _mm_rcp_ps(_element_array.m_xmm_values[0]);
#else
    _element_array.m_xmm_values[0] =
        _mm_div_ps(_mm_set_ss(1.0f), _element_array.m_xmm_values[0]);
#endif
  }
  ...
  recipricol(element_array<float, dimension>& _element_array) {
    ...
    for (size_t i = 0; i < xmm_count_ps<dimension>::value; ++i) {
      ...
      _element_array.m_xmm_values[i] =
          _mm_rcp_ps(_element_array.m_xmm_values[i]);
#else
      _element_array.m_xmm_values[i] =
          _mm_div_ps(one, _element_array.m_xmm_values[i]);
#endif
    }
  }
};
