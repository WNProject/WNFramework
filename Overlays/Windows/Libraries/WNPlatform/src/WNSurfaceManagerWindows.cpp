...
#include "WNPlatform/inc/Internal/WNSurfaceWindows.h"
...
WNSurfaceManagerReturnCode::type WNSurfaceManagerWindows::Initialize() {
  ...
  if (!RegisterClassEx(&wcex)) {
    ...
    if (GetClassInfoEx(GetModuleHandle(NULL), WN_WINDOW_CLASS_NAME, &wcex) ==
        0) {
...
WNSurfaceManagerWindows::WNSurfaceManagerWindows() : mPendingHwnd(0) {}

WNSurfaceManagerReturnCode::type WNSurfaceManagerWindows::Release() {
  for (size_t i = 0; i < mMessagePumps.size(); ++i) {
    SendNotifyMessage(
        mMessagePumps[i]->mWindow->GetNativeHandle(), WM_USER, 0, 0);
  }
...
WNSurfaceManagerReturnCode::type WNSurfaceManagerWindows::CreateSurface(
    uint32_t _x, uint32_t _y, uint32_t _width, uint32_t _height,
    wn::memory::intrusive_ptr<surface>& _surface) {
  wn::memory::intrusive_ptr<WNSurfaceWindows> ptr =
      wn::memory::make_intrusive<WNSurfaceWindows, WNSurfaceManagerWindows&>(
          &allocator, *this);
...
LRESULT CALLBACK WNSurfaceManagerWindows::WindowProc(
    HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  WNWindowThreadData* data = reinterpret_cast<WNWindowThreadData*>(
...
void WNSurfaceManagerWindows::MessagePump(
  ...
  manager.mPendingHwnd = CreateWindowEx(0, WN_WINDOW_CLASS_NAME, "",
      WS_OVERLAPPEDWINDOW, _data->mWindow->GetX(), _data->mWindow->GetY(),
      r.right - r.left, r.bottom - r.top, 0, 0, GetModuleHandle(NULL), NULL);
...
WNSurfaceManagerWindows::WNWindowThreadData::WNWindowThreadData(
    wn::memory::intrusive_ptr<WNSurfaceWindows> _wnd)
  : mWindow(_wnd), mExit(false) {}
