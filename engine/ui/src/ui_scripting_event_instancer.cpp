...
Rocket::Core::EventListener* event_instancer::InstanceEventListener(
  ...
  if (m_contexts.find(element->GetOwnerDocument()) == m_contexts.end()) {
    return new event_listener(m_engine, m_log, m_currently_loading_doc, value, element);  
  }
  auto& r = m_contexts[element->GetOwnerDocument()];
  return new event_listener(
      m_engine, m_log, r, value, element);
}
...
event_listener::event_listener(scripting::engine* _engine, logging::log* _log,
  ...
  if (code.Find("this.") == 0) {
    ...
    if (!m_member_callee) {
      _log->log_error("Could not find member callback function ", code.CString());
    }
  } else {
...
void event_listener::ProcessEvent(Rocket::Core::Event& event) {
  if (m_callee) {
    ...
    m_engine->invoke(m_member_callee, m_ui_data.unsafe_ptr(), event.GetTargetElement());
    return;
  }
  m_log->log_warning(
