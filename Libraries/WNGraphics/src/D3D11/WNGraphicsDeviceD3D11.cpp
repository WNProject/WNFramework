...
WNGraphics::WNGraphicsDeviceD3D11::WNGraphicsDeviceD3D11(
...
WNGraphics::WNGraphicsDeviceD3D11::Initialize(
    uint32_t _adapter, uint32_t _output) {
  if (CreateDXGIFactory1(__uuidof(IDXGIFactory1),
          reinterpret_cast<void**>(&mFactory)) != S_OK) {
    Cleanup();
  }

  mInitializationState =
      WNGraphics::WNGraphicsDeviceD3D11::eWNISDXGIFactoryInitialized;
  ...
  mInitializationState =
      WNGraphics::WNGraphicsDeviceD3D11::eWNISDXGIAdapterInitialized;
  ...
  mInitializationState =
      WNGraphics::WNGraphicsDeviceD3D11::eWNISDXGIOutputInitialized;
  ...
  HRESULT create_result = ::D3D11CreateDevice(mAdapter, D3D_DRIVER_TYPE_UNKNOWN,
      NULL, flags, NULL, 0, D3D11_SDK_VERSION, &mDevice, &level, &mContext);

#ifdef _WN_DEBUG
  if (create_result != S_OK) {
    create_result = ::D3D11CreateDevice(mAdapter, D3D_DRIVER_TYPE_UNKNOWN, NULL,
        0, NULL, 0, D3D11_SDK_VERSION, &mDevice, &level, &mContext);
  }
#endif

  if (create_result != S_OK) {
    create_result = ::D3D11CreateDevice(NULL, D3D_DRIVER_TYPE_WARP, NULL, flags,
        NULL, 0, D3D11_SDK_VERSION, &mDevice, &level, &mContext);

#ifdef _WN_DEBUG
    if (create_result != S_OK) {
      create_result = ::D3D11CreateDevice(mAdapter, D3D_DRIVER_TYPE_WARP, NULL,
          0, NULL, 0, D3D11_SDK_VERSION, &mDevice, &level, &mContext);
    }
#endif
  ...
  mInitializationState =
      WNGraphics::WNGraphicsDeviceD3D11::eWNISD3D11DeviceInitialized;
...
uint32_t WNGraphics::WNGraphicsDeviceD3D11::GetCapability(
    WNGraphics::WNDeviceCaps _cap) {
  switch (_cap) {
...
WNGraphics::WNShader* WNGraphics::WNGraphicsDeviceD3D11::CreateShader(
...
WNGraphics::WNBuffer* WNGraphics::WNGraphicsDeviceD3D11::CreateBuffer(
...
WNGraphics::WNTexture* WNGraphics::WNGraphicsDeviceD3D11::CreateTexture(
...
WNGraphics::WNRenderTarget*
WNGraphics::WNGraphicsDeviceD3D11::CreateRenderTarget(
...
WNGraphics::WNGraphicsDeviceReturnCode::type
WNGraphics::WNGraphicsDeviceD3D11::BindSurface(
  ...
  const HRESULT hr = mOutput->GetDisplayModeList(
      DXGI_FORMAT_B8G8R8A8_UNORM, DXGI_ENUM_MODES_INTERLACED, &numModes, NULL);

  if (S_OK == hr) {
    if (mOutput->GetDisplayModeList(DXGI_FORMAT_B8G8R8A8_UNORM,
            DXGI_ENUM_MODES_INTERLACED, &numModes, NULL) != S_OK) {
      return (WNGraphics::WNGraphicsDeviceReturnCode::eWNGDEDisplayModeError);
    }

    DXGI_MODE_DESC* modeDesc = static_cast<DXGI_MODE_DESC*>(
        WN_STACK_ALLOC(sizeof(DXGI_MODE_DESC) * numModes));

    if (mOutput->GetDisplayModeList(DXGI_FORMAT_B8G8R8A8_UNORM,
            DXGI_ENUM_MODES_INTERLACED, &numModes, modeDesc) != S_OK) {
      return (WNGraphics::WNGraphicsDeviceReturnCode::eWNGDEDisplayModeError);
    }
    ...
    for (uint32_t i = 0; i < static_cast<uint32_t>(numModes); ++i) {
      if (modeDesc[i].Width == static_cast<UINT>(width) &&
          modeDesc[i].Height == static_cast<UINT>(height)) {
    ...
    // We are remote desktop (this is most likely for testing, if you are using
    // this for REAL .. I make no promises
    mScreenWidth = width;
    mScreenHeight = height;
    mScreenNumerator = 1;
  ...
  if (mFactory->CreateSwapChain(mDevice, &swapChainDesc, &dat->mSwapChain) !=
      S_OK) {
    return (
        WNGraphics::WNGraphicsDeviceReturnCode::eWNGDESwapChainCreationError);
  }
  ...
  if (dat->mSwapChain->ResizeBuffers(0, _surface->GetWidth(),
          _surface->GetHeight(), DXGI_FORMAT_UNKNOWN,
          DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH) != S_OK) {
    dat->mSwapChain->Release();
    dat->mSwapChain = nullptr;
  ...
  if (dat->mSwapChain->GetBuffer(
          0, __uuidof(ID3D11Texture2D), (LPVOID*)&backBufferTexture) != S_OK) {
    dat->mSwapChain->Release();
    dat->mSwapChain = nullptr;
  ...
  if (mDevice->CreateRenderTargetView(
          backBufferTexture, NULL, &dat->mBackBufferView) != S_OK) {
  ...
  backBufferTexture->Release();
  _surface->RegisterCallback(
      wn::surface::eWNSETResize,
      wn::surface::WNSurfaceEventCallback(
          &WNGraphicsDeviceD3D11::ProcessWindowMessage, this));
  _surface->RegisterCallback(
      wn::surface::eWNSETDestroyed,
      wn::surface::WNSurfaceEventCallback(
          &WNGraphicsDeviceD3D11::ProcessWindowMessage, this));
...
WNGraphics::WNGraphicsDeviceReturnCode::type
WNGraphics::WNGraphicsDeviceD3D11::SetDrawList(WNGraphics::WNDrawList* _list) {
...
WNGraphics::WNGraphicsDeviceReturnCode::type
WNGraphics::WNGraphicsDeviceD3D11::StartDraw() {
  return (WNGraphics::WNGraphicsDeviceReturnCode::ok);
}

WNGraphics::WNGraphicsDeviceReturnCode::type
WNGraphics::WNGraphicsDeviceD3D11::EndDraw() {
  if (!mActiveSurface) {
    return (WNGraphics::WNGraphicsDeviceReturnCode::eWNGDENoSurface);
  }

  WNWindowData* dat = mActiveSurface->GetRegisteredData<WNWindowData>(
      wn::surface::eWNRDTGraphics);
...
WNGraphics::WNGraphicsDeviceReturnCode::type
WNGraphics::WNGraphicsDeviceD3D11::SubmitDrawList(
    WNGraphics::WNDrawList* _list) {
  WN_UNUSED_ARGUMENT(_list);

  return (WNGraphics::WNGraphicsDeviceReturnCode::ok);
}

bool WNGraphics::WNGraphicsDeviceD3D11::ProcessWindowMessage(
  ...
  if (_message == wn::surface::eWNSETResize) {
    ...
    WNWindowData* dat =
        _surface->GetRegisteredData<WNWindowData>(wn::surface::eWNRDTGraphics);
...
WNGraphics::WNGraphicsDeviceReturnCode::type
WNGraphics::WNGraphicsDeviceD3D11::BindShader(WNGraphics::WNShader* _resource) {
...
WNGraphics::WNGraphicsDeviceReturnCode::type
WNGraphics::WNGraphicsDeviceD3D11::BindBuffer(
...
WNGraphics::WNGraphicsDeviceReturnCode::type
WNGraphics::WNGraphicsDeviceD3D11::BindTexture(
...
WNGraphics::WNGraphicsDeviceReturnCode::type
WNGraphics::WNGraphicsDeviceD3D11::SetActiveSurface(
...
WNGraphics::WNGraphicsDeviceReturnCode::type
WNGraphics::WNGraphicsDeviceD3D11::BindRenderTarget(
...
WNGraphics::WNGraphicsDeviceReturnCode::type
WNGraphics::WNGraphicsDeviceD3D11::Clear() {
  if (mActiveSurface &&
      mActiveSurface
          ->GetRegisteredData<WNWindowData>(wn::surface::eWNRDTGraphics)
          ->mBackBufferView) {
...
WNGraphics::WNGraphicsDeviceReturnCode::type
WNGraphics::WNGraphicsDeviceD3D11::ResizeSwapChain() {
  ...
  if (dat->mRequestedFullScreen != dat->mFullScreen) {
    ...
    if (dat->mSwapChain->SetFullscreenState(
            dat->mRequestedFullScreen, mOutput) != S_OK) {
  ...
  if (dat->mSwapChain->ResizeBuffers(1, dat->mCurrentWidth, dat->mCurrentHeight,
          DXGI_FORMAT_UNKNOWN,
          DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH) != S_OK) {
    dat->mForceResize--;
