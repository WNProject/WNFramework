...
WNGraphics::WNGraphicsDeviceReturnCode::type WNGraphics::WNGraphicsDeviceD3D11::Initialize(wn_uint32 _adapter, wn_uint32 _output) {
    ...
    HRESULT create_result = ::D3D11CreateDevice(mAdapter, D3D_DRIVER_TYPE_UNKNOWN, NULL, flags, NULL, 0, D3D11_SDK_VERSION, &mDevice, &level, &mContext);

#ifdef _WN_DEBUG
    if (create_result != S_OK) {
      create_result = ::D3D11CreateDevice(mAdapter, D3D_DRIVER_TYPE_UNKNOWN, NULL, 0, NULL, 0, D3D11_SDK_VERSION, &mDevice, &level, &mContext);
    }
#endif

    if (create_result != S_OK) {
      create_result = ::D3D11CreateDevice(NULL, D3D_DRIVER_TYPE_WARP, NULL, flags, NULL, 0, D3D11_SDK_VERSION, &mDevice, &level, &mContext);

#ifdef _WN_DEBUG
      if (create_result != S_OK) {
        create_result = ::D3D11CreateDevice(mAdapter, D3D_DRIVER_TYPE_WARP, NULL, 0, NULL, 0, D3D11_SDK_VERSION, &mDevice, &level, &mContext);
      }
#endif
...
wn_bool WNGraphics::WNGraphicsDeviceD3D11::ProcessWindowMessage(wn::surface::WNSurfaceEventType _message, wn::surface* _surface) {
    if (_message == wn::surface::eWNSETResize) {
        std::lock_guard<wn::concurrency::recursive_mutex> mScopeLock(mResizeMutex);
...
WNGraphics::WNGraphicsDeviceReturnCode::type WNGraphics::WNGraphicsDeviceD3D11::ResizeSwapChain() {
    ...
    std::lock_guard<wn::concurrency::recursive_mutex> scopeLock(mResizeMutex);
