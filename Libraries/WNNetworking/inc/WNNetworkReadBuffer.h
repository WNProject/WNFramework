...
public:
  typedef std::deque<wn::memory::intrusive_ptr<WNNetworking::WNBufferResource>>
      WNBufferQueue;
...
public:
  ...
  void AppendBuffer(wn::memory::intrusive_ptr<WNBufferResource>& _buffer,
      size_t _dataCount, size_t _dataOffset);
  bool Initialized();
  void Clear();
  char* GetLastBuffer();
  void PrepareRead();
