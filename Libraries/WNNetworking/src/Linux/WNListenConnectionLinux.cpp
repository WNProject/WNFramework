...
WNListenConnectionLinux::WNListenConnectionLinux(WNNetworkManager& _manager,
...
WNNetworkManagerReturnCode::type WNListenConnectionLinux::Initialize() {
  WN_RELEASE_ASSERT_DESC(mType == WNConnectionType::eWNReliable,
  ...
  const int err =
      setsockopt(mSockFD, SOL_SOCKET, SO_LINGER, &flush, sizeof(flush));
...
WNInConnectionLinux* WNListenConnectionLinux::ReceiveConnection() {
  ...
  WNInConnectionLinux* conn =
      wn::memory::construct<WNInConnectionLinux>(mManager);
