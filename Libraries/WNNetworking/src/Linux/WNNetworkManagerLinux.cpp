...
WNNetworkManagerReturnCode::type WNNetworkManagerLinux::Initialize(
...
#ifdef _WN_ANDROID
  mListenEPollInstance = epoll_create(2048);  // afaik the size isn't even used
                                              // any more (android does not have
                                              // epoll_create1
#else
...
void WNNetworkManagerLinux::ListenThread() {
  ...
  while (1) {
    ...
    for (int32_t i = 0; i < numEvents; ++i) {
      ...
      WNListenConnectionLinux* conn =
          reinterpret_cast<WNListenConnectionLinux*>(events[i].data.ptr);
...
void WNNetworkManagerLinux::ReadThread() {
  ...
  for (;;) {
    ...
    for (int32_t i = 0; i < numEvents; ++i) {
      ...
      WNConnectionLinux* conn =
          reinterpret_cast<WNConnectionLinux*>(events[i].data.ptr);
...
WNNetworkManagerReturnCode::type WNNetworkManagerLinux::CreateListener(
    WNConnection*& _outHandle, WNConnectionType::type _type, uint16_t _port,
    const WNConnectedCallback& _callback) {
  WNListenConnectionLinux* conn =
      wn::memory::construct<WNListenConnectionLinux>(
  ...
  if (epoll_ctl(mListenEPollInstance, EPOLL_CTL_ADD, conn->GetLinuxSocket(),
          &event) != 0) {
    wn::memory::destroy(conn);
...
bool WNNetworkManagerLinux::AddToReadEPoll(WNConnectionLinux* _conn) {
  ...
  if (epoll_ctl(mReadEPollInstance, EPOLL_CTL_ADD, _conn->GetLinuxSocket(),
          &event) == -1) {
    return (false);
  }
...
bool WNNetworkManagerLinux::AddToWriteEPoll(WNConnectionLinux* _conn) {
  ...
  if (epoll_ctl(mWriteEPollInstance, EPOLL_CTL_ADD, _conn->GetLinuxSocket(),
          &event) == -1) {
    return (false);
  }
...
WNNetworkManagerReturnCode::type WNNetworkManagerLinux::ConnectTo(
    WNConnection*& _outHandle, WNConnectionType::type _type,
    const char* _target, uint16_t _port) {
  WN_RELEASE_ASSERT_DESC(_type == WNConnectionType::eWNReliable,
      "WNNetworking does not support unreliable connections .... YET");

  WNOutConnectionLinux* conn =
      wn::memory::construct<WNOutConnectionLinux>(*this);
  const WNNetworkManagerReturnCode::type err =
      conn->Initialize(_type, _target, _port);
...
void WNNetworkManagerLinux::DestroyConnection(WNConnection* _connection) {
  {
    ...
    std::list<WNConnectionLinux*>::iterator i =
...

  {
    ...
    std::list<WNConnectionLinux*>::iterator i = std::find(
...

  {
    ...
    std::list<WNListenConnectionLinux*>::iterator i = std::find(
        mListenConnections.begin(), mListenConnections.end(), _connection);
...
void WNNetworkManagerLinux::CleanAllConnections() {
  {
    ...
    for (std::list<WNConnectionLinux*>::iterator i =
             mIncommingConnections.begin();
         i != mIncommingConnections.end(); ++i) {
      wn::memory::destroy((*i));
    }
...

  {
    ...
    for (std::list<WNConnectionLinux*>::iterator i =
             mOutgoingConnections.begin();
         i != mOutgoingConnections.end(); ++i) {
      wn::memory::destroy((*i));
    }
...

  {
    ...
    for (std::list<WNListenConnectionLinux*>::iterator i =
             mListenConnections.begin();
         i != mListenConnections.end(); ++i) {
      wn::memory::destroy((*i));
    }
