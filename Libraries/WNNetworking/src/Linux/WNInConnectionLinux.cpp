...
wn_bool WNInConnectionLinux::Initialize(
    wn_int32 _yourSocket, sockaddr& _addr, socklen_t addrLen) {
  mSockFD = _yourSocket;

  wn_char name[NI_MAXHOST];
  wn_char port[NI_MAXSERV];

  wn_bool validName = (getnameinfo(&_addr, addrLen, name, NI_MAXHOST, port,
