...
void WNNetworking::WNNetworkManager::SetCallback(uint32_t _identifier, const WNMessageCallback& _callback) {
    if(mCallbackMap.find(_identifier) == mCallbackMap.end()) {
        mCallbackMap[_identifier] = _callback;
    }
}

void WNNetworking::WNNetworkManager::FireCallback(uint32_t _identifier, WNConnection* ptr, WNNetworkReadBuffer& _buffer) {
    WN_UNORDERED_MAP<uint32_t, WNMessageCallback >::iterator it = mCallbackMap.find(_identifier);

    //DO SOME LOGGING HERE
    (*it).second(ptr, _buffer);
}

WNNetworking::WNNetworkManagerReturnCode::type WNNetworking::WNNetworkManager::CreateConnectionGroup(WNConnectionGroup*& _outHandle, const char* _groupName) {
...
void WNNetworking::WNNetworkManager::DestroyConnectionGroup(WNConnectionGroup* _group) {
...
void WNNetworking::WNNetworkManager::UnregisterConnection(WNConnection* _connection)
{
...
void WNNetworking::WNNetworkManager::InitializeBuffer(WNNetworkWriteBuffer& _buffer, const uint32_t _number) {
    uint32_t number1 = 0;
