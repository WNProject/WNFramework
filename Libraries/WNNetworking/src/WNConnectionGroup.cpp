...
WNConnectionGroup::WNConnectionGroup(const char* _groupName) {
  WN_DEBUG_ASSERT_DESC(wn::memory::strlen(_groupName) < 255,
      "Your group name must be < 255 characters long");
...
void WNConnectionGroup::AddConnection(WNConnection* _connection) {
  WN_DEBUG_ASSERT_DESC(std::find(mConnections.begin(), mConnections.end(),
                           _connection) == mConnections.end(),
      "This connection is has already been added");

  mConnections.push_back(_connection);
}

void WNConnectionGroup::CleanConnection(WNConnection* _connection) {
  // If we find this is happening a LOT, should probably switch off to a
  // std::list
  std::vector<WNConnection*>::iterator i =
      std::find(mConnections.begin(), mConnections.end(), _connection);
