...
static wn_void Message(
    WNConnection* _connection, WNNetworkReadBuffer& _buffer) {
...
wn_int32 wn_main(wn_int32 _argc, wn_char* _argv[]) {
  ...
  manager->CreateListener(conn, WNConnectionType::eWNReliable,
      static_cast<wn_uint16>(WN_PORT), WNConnectedCallback(&ConnectedCallback));
  manager2->ConnectTo(
      conn1, WNConnectionType::eWNReliable, WN_ADDRESS, WN_PORT);
  manager2->ConnectTo(
      conn2, WNConnectionType::eWNReliable, WN_ADDRESS, WN_PORT);
