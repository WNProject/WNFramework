...
public:
  ...
  WN_FORCE_INLINE semaphore(const wn_uint16 count) {
    #ifdef _WN_WINDOWS
      m_handle = ::CreateSemaphoreW(NULL, static_cast<LONG>(count),
                                    LONG_MAX, NULL);

      WN_RELEASE_ASSERT_DESC(m_handle != NULL,
        "failed to create semaphore object");
    #elif defined _WN_POSIX
      const int result = ::sem_init(&m_semaphore, 0,
  ...
  WN_FORCE_INLINE ~semaphore() {
      ...
      WN_DEBUG_ASSERT_DESC(result != FALSE,
        "failed to destroy semaphore object");
  ...
  WN_FORCE_INLINE wn_void wait() {
      ...
      WN_RELEASE_ASSERT_DESC(result == WAIT_OBJECT_0,
        "failed to wait on semaphore object");
    #elif defined _WN_POSIX
      int result;

      while ((result = ::sem_wait(&m_semaphore)) == -1) {
        WN_RELEASE_ASSERT_DESC(errno == EINTR,
          "failed to wait on semaphore object");
      }
    #endif
  }
  ...
  WN_FORCE_INLINE wn_bool try_wait() {
      ...
      while ((result = ::sem_trywait(&m_semaphore)) == -1 && errno != EAGAIN) {
        WN_RELEASE_ASSERT_DESC(errno == EINTR,
          "failed to try to wait on semaphore object");
      }
  ...
  WN_FORCE_INLINE wn_void notify(const wn_uint16 count) {
    WN_DEBUG_ASSERT_DESC(count != 0,
      "Posted a semaphore object with a count of 0");

    #ifdef _WN_WINDOWS
      if (count > 0) {
        const BOOL result = ::ReleaseSemaphore(m_handle,
                                               static_cast<LONG>(count),
                                               NULL);

        WN_RELEASE_ASSERT_DESC(result != FALSE,
          "failed to post desired count to semaphore object");
      }
    #elif defined _WN_POSIX
      for (wn_uint16 i = 0; i < count; ++i) {
