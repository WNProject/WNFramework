...
eWNTypeError WNTypeManagerImpl::GetArrayOf(WNScriptType& _type, WNScriptType& _outType) {
    ...
    if((ok != GetTypeByName("-Null", nullType)) || (ok != GetTypeByName("Bool", boolType)) || (ok != GetTypeByName("Int", intType)) || (ok != GetTypeByName("-Function", functionType)) || (ok != GetTypeByName("-Array", arrayType)))
    {
...
eWNTypeError GenerateStructDestruction::Execute(WNCodeModule& _module, WNScriptType _node, llvm::Value* _structLoc, const WNFunctionDefinition* _def, WNLogging::WNLog& _compilationLog) const {
    ...
    if(ok != _module.GetTypeManager().GetTypeByName("-Ptr", voidStarType)) {
        return(eWNPlatformError);
    }
    ...
    if(ok != _module.GetTypeManager().GetTypeByName("Void", voidReturnType)) {
