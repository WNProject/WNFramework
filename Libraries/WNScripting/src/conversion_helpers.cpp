...
parse_ast_convertor::convertor_context::get_id(
  ...
  if (m_current_function->m_is_member_function) {
    auto st = m_current_function->m_parameters[0].m_type->m_is_synchronized
                  ? m_current_function->m_parameters[0].m_type
                  : m_current_function->m_parameters[0]
                        .m_type->m_implicitly_contained_type;
    uint32_t num = 0;
    ...
    for (auto& f : st->m_member_functions) {
      if (f->m_name == _name) {
        if (m_current_function->m_is_synchronized &&
            !(f->m_is_synchronized || f->m_action_function)) {
          _location->log_line(m_log, logging::log_level::error);
          m_log->log_error("Cannot call non-synchronized function '", _name,
              "' from within a synchronized one");
          return nullptr;
        }
        auto mae = memory::make_unique<ast_member_access_expression>(
            m_allocator, _location);
