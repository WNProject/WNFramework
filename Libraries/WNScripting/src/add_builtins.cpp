...
bool type_manager::add_builtin_functions() {
  ...
  if (m_externals_by_name.find(containers::string(
          m_allocator, "_ZN3wns15_allocate_actorEPvN3wns4sizeE")) == m_externals_by_name.end()) {
  ...
  m_call_actor_function = m_externals_by_name["_ZN3wns20_call_actor_functionEvPv"];
  add_allocate_shared();
  add_release_shared();
  add_assign_shared();
  add_return_shared();
...
void type_manager::add_allocate_actor() {
  ...
  cast_to_soh->m_base_expression =
      call_function(nullptr, alloc, core::move(params));
  cast_to_soh->m_type = get_reference_of(m_actor_header.get(),
      ast_type_classification::reference, nullptr);
...
void type_manager::add_release_actor() {
  ...
  auto shr_obj =
      memory::make_unique<ast_builtin_expression>(m_allocator, nullptr);
  shr_obj->m_type = get_reference_of(m_actor_header.get(),
      ast_type_classification::reference, nullptr);
  shr_obj->initialized_expressions(m_allocator)
      .push_back(clone_ast_node(m_allocator, in_param.get()));
