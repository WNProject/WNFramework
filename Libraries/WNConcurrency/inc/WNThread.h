...
namespace wn {
    ...
    public:
        ...
        WN_FORCE_INLINE explicit thread(_Function&& _function, _Arguments&&... _arguments) {
            static_assert(is_same<result_type, result_of_t<_Function(_Arguments...)>>::value,
                          "Function return type does not match thread return type");
    ...
    namespace this_thread {
        ...
        WN_INLINE wn_void sleep_for(const std::chrono::duration<representation, period>& _sleep_duration) {
            #ifdef _WN_WINDOWS
                const auto milli_sec = std::chrono::duration_cast<std::chrono::milliseconds>(_sleep_duration).count();

                ::Sleep(static_cast<DWORD>(milli_sec));
            #elif defined _WN_POSIX
                const auto sec = std::chrono::duration_cast<std::chrono::seconds>(_sleep_duration);
                const auto nano_sec = (std::chrono::duration_cast<std::chrono::nanoseconds>(_sleep_duration) - 
