...
namespace wn {
    namespace core {
        namespace internal {
            WN_INLINE wn_void assert_helper(const wn_int32 _argument, const wn_char* _file, wn_uint32 _line, const wn_char* _message) {
                if (_argument == 0) {
                    #if defined _WN_WINDOWS && defined _WN_CORE_CONFIG_WINDOWS_ENABLE_ASSERT_POPUPS
                        TCHAR display[1024];
                        ...
                        ::GetModuleFileName(NULL, program, MAX_PATH);
                            ...
                            ::_snwprintf_s(display, 1024, L"Assertion failed!\n\nProgram: %s\nFile: %s\nLine: %d\n\nExpression: %s", program, file, _line, message);
                        #else
                            ::_snprintf_s(display, 1024, "Assertion failed!\n\nProgram: %s\nFile: %s\nLine: %d\n\nExpression: %s", program, _file, _line, _message);
                        #endif
