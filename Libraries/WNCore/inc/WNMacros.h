...
#define WN_FOREACH_1(CALL, PARAM, X) \
  CALL(PARAM, X)
...
#define RETURN_ARG_COUNT(_1,_2,_3,_4,_5,_6,_7,_8,_9,_10,count,...) count
...
#define FOR_EACH(name,param,...) \
  REMOVE_COMMA(RUN_FOREACH(COUNT_ARGS_MAX10(__VA_ARGS__)), \
    (name,param,__VA_ARGS__))
