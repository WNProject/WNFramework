...
WNFileSystem::WNWriteTextFileBuffer::WNWriteTextFileBuffer(
  ...
  : WNFileBuffer(_type), mBufferUsage(0), mBufferSize(_bufferSize) {
...
char* WNFileSystem::WNWriteTextFileBuffer::reserve(
...
wn::containers::data_buffer_type WNFileSystem::WNWriteTextFileBuffer::type()
    const {
  switch (mType) {
...
WNFileSystem::WNFile::WNFileError WNFileSystem::WNWriteTextFileBuffer::SetFile(
    const char* _fileName) {
  WN_RELEASE_ASSERT_DESC(!mFile.IsValid(),
      "Error, trying to set a file on a text buffer that already exists");

  return (mFile.OpenFile(_fileName, WNFileSystem::WNFile::eWNFMClobber |
                                        WNFileSystem::WNFile::eWNFMExclusive |
                                        WNFileSystem::WNFile::eWNFMWrite |
                                        WNFileSystem::WNFile::eWNFMCreate |
                                        WNFileSystem::WNFile::eWNFMStream));
}

bool WNFileSystem::WNWriteTextFileBuffer::serialize(
