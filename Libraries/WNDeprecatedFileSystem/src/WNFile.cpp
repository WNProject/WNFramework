...
WNFileSystem::WNFile::WNFileError WNFileSystem::WNFile::OpenFile(
    const char* _name, uint32_t _mode) {
  WN_DEBUG_ASSERT(!mIsFileOpen);

  mStream = (_mode & WNFileSystem::WNFile::eWNFMStream) ==
            WNFileSystem::WNFile::eWNFMStream;

  if ((_mode & (WNFileSystem::WNFile::eWNFMCreate |
  ...
  accessType |=
      ((_mode & WNFileSystem::WNFile::eWNFMWrite) ? GENERIC_WRITE : 0);

  const DWORD sharing = (_mode & WNFileSystem::WNFile::eWNFMExclusive)
                            ? 0
                            : (FILE_SHARE_READ | FILE_SHARE_WRITE);
  DWORD creationDisposition =
      (_mode & WNFileSystem::WNFile::eWNFMCreate) ? OPEN_ALWAYS : OPEN_EXISTING;
  ...
  mFileHandle = CreateFile(
      _name, accessType, sharing, nullptr, creationDisposition, 0, 0);
  ...
  if (!mStream) {
    ...
    if (static_cast<LONGLONG>(static_cast<size_t>(filesize.QuadPart)) !=
        filesize.QuadPart) {
      CloseHandle(mFileHandle);
...
#elif defined _WN_LINUX || defined _WN_ANDROID
  if (_mode & WNFileSystem::WNFile::eWNFMClobber) {
    if ((_mode & WNFileSystem::WNFile::eWNFMRead) !=
        WNFileSystem::WNFile::eWNFMRead) {
      mFileHandle = fopen(_name, "w");
    } else {
      mFileHandle = fopen(_name, "r+");
    }
  } else if ((_mode & WNFileSystem::WNFile::eWNFMRead) &&
             (_mode & WNFileSystem::WNFile::eWNFMWrite)) {
    mFileHandle = fopen(_name, "r+");
  } else if ((_mode & WNFileSystem::WNFile::eWNFMRead)) {
    mFileHandle = fopen(_name, "r");
...
char* WNFileSystem::WNFile::GetFileName(const char* _name) {
  ...
  return (
      wn::memory::strndup(_name + loc, wn::memory::strlen(_name) - loc + 1));
}
...
char* WNFileSystem::WNFile::GetDataBuffer() {
  WN_DEBUG_ASSERT_DESC(
      !mStream, "You cannot get a data buffer for a streamed file");
  ...
  do {
    const DWORD maxRead = (static_cast<DWORD>(readLeft) == readLeft)
                              ? static_cast<DWORD>(readLeft)
                              : -1;
    DWORD amountRead = 0;

    if (!ReadFile(mFileHandle, mFileBuffer + (mFileSize - readLeft), maxRead,
            &amountRead, nullptr)) {
      wn::memory::heap_free(mFileBuffer);
  ...
  if (fread(mFileBuffer, sizeof(char), mFileSize,
          static_cast<FILE*>(mFileHandle)) != mFileSize) {
    wn::memory::heap_free(mFileBuffer);
...
bool WNFileSystem::WNFile::CommitFileBuffer() {
  ...
  do {
    const DWORD maxWrite = (static_cast<DWORD>(writeLeft) == writeLeft)
                               ? static_cast<DWORD>(writeLeft)
                               : -1;
    DWORD amountWrote = 0;

    if (!WriteFile(mFileHandle, mFileBuffer + (mFileSize - writeLeft), maxWrite,
            &amountWrote, nullptr)) {
      return (false);
    }
...
size_t WNFileSystem::WNFile::ReadData(char* _outBuffer, size_t _amount) {
  ...
  do {
    ...
    if (!ReadFile(mFileHandle, _outBuffer + (_amount - readLeft), maxRead,
            &amountRead, nullptr)) {
  ...
  const size_t amountRead =
      fread(_outBuffer, sizeof(char), _amount, mFileHandle);
...
size_t WNFileSystem::WNFile::WriteData(const char* _inBuffer, size_t _amount) {
  ...
  do {
    ...
    if (!WriteFile(mFileHandle, _inBuffer + (_amount - writeLeft), maxWrite,
            &amountWrote, nullptr)) {
      return (0);
    }
  ...
  return (totalWritten);
#elif defined _WN_LINUX || defined _WN_ANDROID
  const size_t amountWritten =
      fwrite(_inBuffer, sizeof(char), _amount, mFileHandle);
