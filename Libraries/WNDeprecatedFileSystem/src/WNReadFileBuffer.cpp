...
WNFileSystem::WNReadTextFileBuffer::WNReadTextFileBuffer(
    const WNFileSystem::WNFileBufferType _type, const size_t _bufferSize)
  : WNFileBuffer(_type),
    mBufferUsage(0),
...
char* WNFileSystem::WNReadTextFileBuffer::reserve(
  ...
  if (_numBytes + mBufferPosition > mBufferSize) {
    if (_numBytes > mBufferSize) {
      ...
      wn::memory::memcpy(mSpareBuffer, mCurrentBuffer + mBufferPosition,
  ...
  if (!mEndFile && haveData < _numBytes) {
    const size_t bytesRead =
        mFile.ReadData(mCurrentBuffer + mBufferUsage, _numBytes - haveData);
...
wn::containers::data_buffer_type WNFileSystem::WNReadTextFileBuffer::type()
    const {
  switch (mType) {
...
WNFileSystem::WNFile::WNFileError WNFileSystem::WNReadTextFileBuffer::SetFile(
    const char* _fileName) {
  mEndFile = false;

  WN_RELEASE_ASSERT_DESC(!mFile.IsValid(),
      "Error, trying to set a file on a text buffer that already exists");

  return (mFile.OpenFile(_fileName, WNFileSystem::WNFile::eWNFMExclusive |
                                        WNFileSystem::WNFile::eWNFMRead |
                                        WNFileSystem::WNFile::eWNFMStream));
}

bool WNFileSystem::WNReadTextFileBuffer::serialize(
