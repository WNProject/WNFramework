...
namespace wn {
    namespace memory {
        ...
        WN_FORCE_INLINE _Type* malloc() {
            static_assert(!std::is_void<_Type>::value,
                          "you must specify a size in bytes when trying to allocate memory of type void");

            return(malloc<_Type>(1));
        }

        template <typename _Type>
        WN_FORCE_INLINE _Type* malloc(const wn_size_t _size) {
            return(static_cast<_Type*>(malloc<wn_void>(_size * sizeof(_Type))));
        }
        ...
        template <typename _Type>
        WN_FORCE_INLINE _Type* realloc(_Type* _ptr, const wn_size_t _new_size) {
        ...
        template <typename _Type>
        WN_FORCE_INLINE wn_void free(_Type* _ptr) {
        ...
        template <typename _Type>
        WN_FORCE_INLINE _Type* aligned_malloc(const wn_size_t _alignment) {
            return(aligned_malloc<_Type>(1, _alignment));
        }

        template <typename _Type>
        WN_FORCE_INLINE _Type* aligned_malloc(const wn_size_t _size, const wn_size_t _alignment) {
            return(static_cast<_Type*>(aligned_malloc<wn_void>(_size * sizeof(_Type), _alignment)));
        }
        ...
        template <typename _Type>
        WN_FORCE_INLINE _Type* aligned_realloc(_Type* _ptr, const wn_size_t _new_size, const wn_size_t _alignment) {
            wn_void* ptr = static_cast<wn_void*>(_ptr);
        ...
        template <typename _Type>
        WN_FORCE_INLINE wn_void aligned_free(_Type* _ptr) {
