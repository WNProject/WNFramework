...
namespace wn {
    namespace memory {
        ...
        template <typename _Type>
        using unique_ptr = std::unique_ptr<_Type, deleter<_Type> >;
        ...
        template <typename _Type, typename... _Args>
        WN_FORCE_INLINE unique_ptr<_Type> make_unique(_Args&&... _args) {
          return unique_ptr<_Type>(new(std::nothrow) _Type(std::forward<_Args>(_args)...));
        }
    }
}
