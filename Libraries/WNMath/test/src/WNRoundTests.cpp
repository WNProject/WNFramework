...
TEST(WNRoundFLOAT32, ZeroTest) {
    const WN_FLOAT32 result = WNRound(0.0f);

    ASSERT_EQ(result, 0.0f);
}

TEST(WNRoundFLOAT32, ValueTest) {
    for (WN_UINT32 i = 0; i < NUMBER_VALUES; ++i) {
        const WN_FLOAT32 number = static_cast<WN_FLOAT32>(i);
        const WN_FLOAT32 custom = WNRound(number);
        const WN_FLOAT32 standard = floor(number + 0.5f);
...
TEST(WNRoundFLOAT64, ZeroTest) {
    const WN_FLOAT64 result = WNRound(0.0);

    ASSERT_EQ(result, 0.0);
}

TEST(WNRoundFLOAT64, ValueTest) {
    for (WN_UINT32 i = 0; i < NUMBER_VALUES; ++i) {
        const WN_FLOAT64 number = static_cast<WN_FLOAT64>(i);
        const WN_FLOAT64 custom = WNRound(number);
        const WN_FLOAT64 standard = floor(number + 0.5);
