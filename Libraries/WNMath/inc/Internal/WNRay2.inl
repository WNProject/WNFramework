...
#error                                                                         \
    "WNRay2.inl should never be included directly. Please include WNRay2.h instead"
#endif
...
WNRay2<type>::WNRay2(const type* _numbers)
  : mLocation(&_numbers[0]), mDirection(&_numbers[2]) {}

template <typename type>
WNRay2<type>::WNRay2(
    const WNVector2<type>& _location, const WNVector2<type>& _direction)
...
template <typename type>
bool WNRay2<type>::operator==(const WNRay2<type>& _ray) const {
...
template <typename type>
void WNRay2<type>::Zero() {
  mLocation.Zero();
  mDirection.Zero();
}

template <typename type>
void WNRay2<type>::Set(const type* _numbers) {
  mLocation.Set(&_numbers[0]);
  mDirection.Set(&_numbers[2]);
}

template <typename type>
void WNRay2<type>::Set(
    const WNVector2<type>& _location, const WNVector2<type>& _direction) {
  mLocation = _location;
...
WNLine3<type> WNRay2<type>::ToLine3() const {
  return (
      WNLine3<type>(mLocation.ToPoint3(), (mLocation + mDirection).ToPoint3()));
}
}
