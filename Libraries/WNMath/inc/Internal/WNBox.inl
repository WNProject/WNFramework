...
#error                                                                         \
    "WNBox.inl should never be included directly. Please include WNBox.h instead"
#endif
...
WNBox<type>::WNBox()
  : mLocation(static_cast<type>(0), static_cast<type>(0), static_cast<type>(0)),
    ...
    mRotation(static_cast<type>(0), static_cast<type>(0), static_cast<type>(0),
        static_cast<type>(1)) {}
...
template <typename type>
WNBox<type>::WNBox(const WNVector<type>& _location,
    const WNVector<type>& _extents, const WNQuaternion<type>& _rotation)
...
bool WNBox<type>::operator==(const WNBox& _box) const {
  return (mExtents == _box.mExtents && mLocation == _box.mLocation &&
          mRotation == _box.mRotation);
}
...
template <typename type>
void WNBox<type>::Set(const WNVector<type>& _location, WNVector<type>& _extents,
...
bool WNBox<type>::Expand(const WNVector<type>& _vector, bool _anchor) {
  const WNVector<type> transformedPoint =
      ((_vector - mLocation).GetRotated(mRotation.GetInverted()).ToPoint3());

  if (!_anchor) {
    WNBounds3<type> bounds =
        WNBounds3<type>(-mExtents.ToPoint3(), mExtents.ToPoint3());
...
bool WNBox<type>::Expand(const WNSphere<type>& _sphere, bool _anchor) {
  const WNVector<type> transformedLocation =
      ((_sphere.mLocation - mLocation)
              .GetRotated(mRotation.GetInverted())
              .ToPoint3());
  const WNVector<type> componentX = WNVector<type>(
      _sphere.mRadius, static_cast<type>(0), static_cast<type>(0));
  const WNVector<type> componentY = WNVector<type>(
      static_cast<type>(0), _sphere.mRadius, static_cast<type>(0));
  const WNVector<type> componentZ = WNVector<type>(
      static_cast<type>(0), static_cast<type>(0), _sphere.mRadius);
...
bool WNBox<type>::IsInsideOut() const {
  return (mExtents.mX < static_cast<type>(0) ||
          mExtents.mY < static_cast<type>(0) ||
          mExtents.mZ < static_cast<type>(0));
}
...
template <typename type>
WNBox<type> WNBox<type>::GetExpanded(
    const WNVector<type>& _vector, bool _anchor) const {
  WNBox<type> box = *this;
...
template <typename type>
WNBox<type> WNBox<type>::GetExpanded(
    const WNSphere<type>& _sphere, bool _anchor) const {
  WNBox<type> box = *this;
...
template <typename type>
WNBox<type> WNBox<type>::GetExpanded(
    const WNBounds3<type>& _bounds, bool _anchor) const {
  WNBox<type> box = *this;
...
void WNBox<type>::GetPoints(WNVector<type>* _vectors) const {
  ...
  const WNVector<type> transformedXExtents =
      (mExtents * WNVector<type>(static_cast<type>(-1), static_cast<type>(1),
                      static_cast<type>(1)))
          .GetRotated(mRotation);
  const WNVector<type> transformedYExtents =
      (mExtents * WNVector<type>(static_cast<type>(1), -static_cast<type>(-1),
                      static_cast<type>(1)))
          .GetRotated(mRotation);
  const WNVector<type> transformedZExtents =
      (mExtents * WNVector<type>(static_cast<type>(1), static_cast<type>(1),
                      -static_cast<type>(-1)))
          .GetRotated(mRotation);
