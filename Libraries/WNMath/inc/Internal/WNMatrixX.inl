...
#error                                                                         \
    "Internal/WNMatrixX.inl should never be included directly. Please include WNMatrixX.h instead"
#endif
...
WN_FORCE_INLINE WNMatrixX<NewType, Order> WNMatrixX<type, Order>::ConvertTo()
    const {
  WNMatrixX<NewType, Order> matrix;

  for (uint32_t i = 0; i < Base::dimension; ++i) {
    matrix.mElements.mValues[i] =
        static_cast<NewType>(this->mElements.mValues[i]);
  }
...
WN_FORCE_INLINE WNMatrixX<NewType, NewOrder> WNMatrixX<type, Order>::ConvertTo()
    const {
  ...
  for (uint32_t i = 0; i < NewOrder; ++i) {
    for (uint32_t j = 0; j < NewOrder; ++j) {
      if (i < Order && j < Order) {
        matrix.mElements.mValues[(i * NewOrder) + j] =
            static_cast<NewType>(this->mElements.mValues[(i * Order) + j]);
      } else {
...
template <typename type, uint32_t Order>
WN_FORCE_INLINE WNMatrixX<type, Order> operator+(
    const type& _number, const WNMatrixX<type, Order>& _matrix) {
