    ...
    namespace internal {
        namespace math {
            struct general_traits_sse : general_traits_generic {
                ...
                static WN_FORCE_INLINE void initialize(element_array<type, dimension>& _element_array) {
                ...
                template <typename type, const size_t dimension>
                static WN_FORCE_INLINE void assign(element_array<type, dimension>& _element_array1,
                ...
                template <typename type, const size_t dimension>
                static WN_FORCE_INLINE void assign(element_array<type, dimension>& _element_array, const type& _value) {
                    base::assign(_element_array, _value);
                }

                template <typename type>
                static WN_FORCE_INLINE typename core::enable_if<core::is_same<type, void>::value>::type assign(element_array<float, 2>& _element_array, const float& _value) {
                    _element_array.m_xmm_values[0] = _mm_setr_ps(_value, _value, 0.0f, 0.0f);
                }

                template <typename type>
                static WN_FORCE_INLINE typename core::enable_if<core::is_same<type, void>::value>::type assign(element_array<float, 3>& _element_array, const float& _value) {
                    _element_array.m_xmm_values[0] = _mm_setr_ps(_value, _value, _value, 0.0f);
                }

                template <typename type>
                static WN_FORCE_INLINE typename core::enable_if<core::is_same<type, void>::value>::type assign(element_array<float, 4>& _element_array, const float& _value) {
                ...
                assign(element_array<float, dimension>& _element_array, const float& _value) {
                ...
                assign(element_array<float, dimension>& _element_array, const float& _value) {
                    ...
                    _element_array.m_xmm_values[xmm_count] = _mm_setr_ps(_value, _value, 0.0f, 0.0f);
                }
                ...
                assign(element_array<float, dimension>& _element_array, const float& _value) {
                    ...
                    _element_array.m_xmm_values[xmm_count] = _mm_setr_ps(_value, _value, _value, 0.0f);
                }

                template <typename type, const size_t dimension, typename iterator>
                static WN_FORCE_INLINE void assign(element_array<type, dimension>& _element_array,
                                                      iterator _first, iterator _last) {
                    base::assign(_element_array, _first, _last);
                }

                template <typename type, const size_t dimension>
                static WN_FORCE_INLINE void assign(element_array<type, dimension>& _element_array, const type* _numbers) {
                    base::assign(_element_array, _numbers);
                }

                template <typename type, const size_t dimension>
                static WN_FORCE_INLINE void add(element_array<type, dimension>& _element_array1,
                                                   const element_array<type, dimension>& _element_array2) {
                ...
                add(element_array<float, dimension>& _element_array1,
                    ...
                    _element_array1.m_xmm_values[0] = _mm_add_ps(_element_array1.m_xmm_values[0],
                                                                 _element_array2.m_xmm_values[0]);
                }
                ...
                add(element_array<float, dimension>& _element_array1,
                    ...
                    for (size_t i = 0; i < xmm_count; ++i) {
                        _element_array1.m_xmm_values[i] = _mm_add_ps(_element_array1.m_xmm_values[i],
                                                                     _element_array2.m_xmm_values[i]);
                    }

                    _element_array1.m_xmm_values[xmm_count] = _mm_add_ss(_element_array1.m_xmm_values[xmm_count],
                ...
                template <typename type, const size_t dimension>
                static WN_FORCE_INLINE void add(element_array<type, dimension>& _element_array, const type& _value) {
                    base::add(_element_array, _value);
                }

                template <typename type>
                static WN_FORCE_INLINE typename std::enable_if<core::is_same<type, void>::value>::type add(element_array<float, 2>& _element_array, const float& _value) {
                    _element_array.m_xmm_values[0] = _mm_add_ps(_element_array.m_xmm_values[0],
                                                                _mm_setr_ps(_value, _value, 0.0f, 0.0f));
                }

                template <typename type>
                static WN_FORCE_INLINE typename std::enable_if<core::is_same<type, void>::value>::type add(element_array<float, 3>& _element_array, const float& _value) {
                    _element_array.m_xmm_values[0] = _mm_add_ps(_element_array.m_xmm_values[0],
                                                                _mm_setr_ps(_value, _value, _value, 0.0f));
                }

                template <typename type>
                static WN_FORCE_INLINE typename std::enable_if<core::is_same<type, void>::value>::type add(element_array<float, 4>& _element_array, const float& _value) {
                    _element_array.m_xmm_values[0] = _mm_add_ps(_element_array.m_xmm_values[0], _mm_set1_ps(_value));
                }

                template <const size_t dimension>
                static WN_FORCE_INLINE typename core::enable_if<(dimension > 4 && dimension % 4 == 1)>::type
                add(element_array<float, dimension>& _element_array, const float& _value) {
                    const __m128 value = _mm_set1_ps(_value);
                    const size_t xmm_count = xmm_count_ps<dimension>::value - 1;

                    for (size_t i = 0; i < xmm_count; ++i) {
                        _element_array.m_xmm_values[i] = _mm_add_ps(_element_array.m_xmm_values[i], value);
                    }

                    _element_array.m_xmm_values[xmm_count] = _mm_add_ss(_element_array.m_xmm_values[xmm_count], value);
                }
