...
struct basic_traits_sse4_1 : basic_traits_sse2 {
  ...
  static WN_FORCE_INLINE typename core::enable_if<
      (is_real<type>::value || std::is_integral<type>::value), type>::type
  mod(const type& _dividend, const type& _divisor) {
    base::mod(_dividend, _divisor);
  }

  template <>
  static WN_FORCE_INLINE float mod(
      const float& _dividend, const float& _divisor) {
    const __m128 divisor = _mm_load_ss(&_divisor);

#ifdef __WN_MATH_APPROXIMATIONS_ENABLED
    const __m128 result =
        _mm_mul_ss(_mm_load_ss(&_dividend), _mm_rcp_ss(divisor));
#else
    const __m128 result = _mm_div_ss(_mm_load_ss(&_dividend), divisor);
#endif

    const __m128 interger_portion =
        _mm_round_ss(_mm_setzero_ps(), result, _MM_FROUND_TRUNC);

    return (_mm_cvtss_f32(
        _mm_mul_ss(_mm_sub_ss(result, interger_portion), divisor)));
  }

  template <>
  static WN_FORCE_INLINE double mod(
      const double& _dividend, const double& _divisor) {
    const __m128d divisor = _mm_load_sd(&_divisor);
    const __m128d result = _mm_div_sd(_mm_load_sd(&_dividend), divisor);
    const __m128d interger_portion =
        _mm_round_sd(_mm_setzero_pd(), result, _MM_FROUND_TRUNC);

    return (_mm_cvtsd_f64(
        _mm_mul_sd(_mm_sub_sd(result, interger_portion), divisor)));
  }

  template <typename type>
  static WN_FORCE_INLINE
      typename core::enable_if<is_real<type>::value, type>::type
  ...
  static WN_FORCE_INLINE float round(const float& _value) {
    return (_mm_cvtss_f32(
        _mm_round_ss(_mm_setzero_ps(), _mm_load_ss(&_value), _MM_FROUND_NINT)));
  }

  template <>
  static WN_FORCE_INLINE double round(const double& _value) {
    return (_mm_cvtsd_f64(
        _mm_round_sd(_mm_setzero_pd(), _mm_load_sd(&_value), _MM_FROUND_NINT)));
  }
  ...
  static WN_FORCE_INLINE float floor(const float& _value) {
    return (
        _mm_cvtss_f32(_mm_floor_ss(_mm_setzero_ps(), _mm_load_ss(&_value))));
  }

  template <>
  static WN_FORCE_INLINE double floor(const double& _value) {
    return (
        _mm_cvtsd_f64(_mm_floor_sd(_mm_setzero_pd(), _mm_load_sd(&_value))));
  }
  ...
  static WN_FORCE_INLINE float trunc(const float& _value) {
    return (_mm_cvtss_f32(_mm_round_ss(
        _mm_setzero_ps(), _mm_load_ss(&_value), _MM_FROUND_TRUNC)));
  }

  template <>
  static WN_FORCE_INLINE double trunc(const double& _value) {
    return (_mm_cvtsd_f64(_mm_round_sd(
        _mm_setzero_pd(), _mm_load_sd(&_value), _MM_FROUND_TRUNC)));
  }
