...
//#error "WNBounds2.inl should never be included directly. Please include
//WNBounds2.h instead"
#endif
...
WNBounds2<type>::WNBounds2(const type* _numbers)
  : mMinimum(_numbers[0], _numbers[1]), mMaximum(_numbers[2], _numbers[3]) {}
...
template <typename type>
WNBounds2<type>::WNBounds2(
    const WNVector2<type>& _min, const WNVector2<type>& _max)
...
type WNBounds2<type>::Perimeter() const {
  ...
  return (WNAbs(static_cast<type>(2) * size.mX) +
          WNAbs(static_cast<type>(2) * size.mY));
}
...
bool WNBounds2<type>::Expand(const WNRectangle<type>& _rectangle) {
  const WNVector2<type> extents =
      _rectangle.mExtents.GetRotated(_rectangle.mRotation);
  const WNVector2<type> point1 = _rectangle.mLocation + extents;
  const WNVector2<type> point2 =
      _rectangle.mLocation + WNVector2<type>(-extents.mX, extents.mY);
  const WNVector2<type> point3 =
      _rectangle.mLocation + WNVector2<type>(extents.mX, -extents.mY);
...
bool WNBounds2<type>::Expand(const WNCircle<type>& _circle) {
  const WNVector2<type> point1 = WNVector2<type>(
      _circle.mLocation.mX + _circle.mRadius, _circle.mLocation.mY);
  const WNVector2<type> point2 = WNVector2<type>(
      _circle.mLocation.mX - _circle.mRadius, _circle.mLocation.mY);
  const WNVector2<type> point3 = WNVector2<type>(
      _circle.mLocation.mX, _circle.mLocation.mY + _circle.mRadius);
  const WNVector2<type> point4 = WNVector2<type>(
      _circle.mLocation.mX, _circle.mLocation.mY - _circle.mRadius);
...
template <typename type>
WNBounds2<type> WNBounds2<type>::GetExpanded(
    const WNVector2<type>& _vector) const {
  WNBounds2 bounds = *this;
...
template <typename type>
WNBounds2<type> WNBounds2<type>::GetExpanded(
    const WNRectangle<type>& _rectangle) const {
...
template <typename type>
WNBounds2<type> WNBounds2<type>::GetExpanded(
    const WNCircle<type>& _circle) const {
  WNBounds2<type> bounds = *this;
...
template <typename type>
WNBounds2<type> WNBounds2<type>::GetExpanded(
    const WNBounds2<type>& _bounds) const {
  WNBounds2<type> bounds = *this;
...
template <typename type>
WNBounds2<type> WNBounds2<type>::GetTranslated(
    const WNVector2<type>& _vector) const {
  WNBounds2<type> bounds = *this;
...
void WNBounds2<type>::GetPoints(WNVector2<type>* _vectors) const {
  _vectors[0] = mMaximum;
  _vectors[1] =
      (mMinimum * WNVector2<type>(static_cast<type>(0), static_cast<type>(1)) +
          mMaximum *
              WNVector2<type>(static_cast<type>(1), static_cast<type>(0)))
          .ToPoint2();
  _vectors[2] =
      (mMinimum * WNVector2<type>(static_cast<type>(1), static_cast<type>(0)) +
          mMaximum *
              WNVector2<type>(static_cast<type>(0), static_cast<type>(1)))
          .ToPoint2();
  _vectors[3] = mMinimum;
}
