...
public:
  ...
  template <typename value_type, typename... types,
      typename = core::enable_if_t<core::bool_and<
          core::are_same<value_type, types..., value_type>::value,
          (sizeof...(types) == (_dimension - 1)), (_order > 1)>::value>>
  WN_FORCE_INLINE explicit matrix(
  ...
  WN_FORCE_INLINE value_type& at() {
    ...
    static_assert(
        _column < _order, "Attempting to get element outside of bounds");
  ...
  WN_FORCE_INLINE value_type& at(const size_t _row, const size_t _column) {
    WN_DEBUG_ASSERT_DESC(
        _row < _order, "Attempting to get element outside of bounds");
    WN_DEBUG_ASSERT_DESC(
        _column < _order, "Attempting to get element outside of bounds");
  ...
  WN_FORCE_INLINE const value_type& at(
      const size_t _row, const size_t _column) const {
    WN_DEBUG_ASSERT_DESC(
        _row < _order, "Attempting to get element outside of bounds");
    WN_DEBUG_ASSERT_DESC(
        _column < _order, "Attempting to get element outside of bounds");
  ...
  WN_FORCE_INLINE void transpose() {
    for (size_t i = 0; i < _order; ++i) {
      ...
      for (size_t j = 0; j < _order; ++j) {
        ...
        std::swap(
            m_element_array.m_values[index], m_element_array.m_values[index]);
      }
    }
  }
  ...
  WN_FORCE_INLINE matrix<value_type, (_order - 1)> extract_sub_matrix(
      const size_t _row, const size_t _column) {
      ...
      if (i != _row) {
        ...
        for (size_t j = 0; j < _order; ++j) {
          if (j != _column) {
            ...
            m_element_array.m_values[index] =
                matrix.m_element_array.m_values[index];
...
wn::matrix<type, order> operator+(
    const type& _value, wn::matrix<type, order>&& _matrix) {
  return (_matrix += _value);
}

template <typename type, const size_t order>
wn::matrix<type, order> operator+(
    const type& _value, const wn::matrix<type, order>& _matrix) {
  return (_matrix + _value);
}
