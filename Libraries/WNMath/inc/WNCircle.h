...
public:
  ...
  WN_FORCE_INLINE circle(const circle& _circle)
    : m_location(_circle.m_location), m_radius(_circle.m_radius) {}

  WN_FORCE_INLINE explicit circle(const value_type* _numbers)
    : m_location(_numbers[0], _numbers[1]), m_radius(_numbers[2]) {}

  WN_FORCE_INLINE explicit circle(
      vector2<value_type>&& _location, value_type&& _radius)
    : m_location(std::move(_location)), m_radius(std::move(_radius)) {}

  WN_FORCE_INLINE explicit circle(
      vector2<value_type>&& _location, const value_type& _radius)
    : m_location(std::move(_location)), m_radius(_radius) {}

  WN_FORCE_INLINE explicit circle(
      const vector2<value_type>& _location, value_type&& _radius)
    : m_location(_location), m_radius(std::move(_radius)) {}

  WN_FORCE_INLINE explicit circle(
      const vector2<value_type>& _location, const value_type& _radius)
    : m_location(_location), m_radius(_radius) {}

  WN_FORCE_INLINE explicit circle(
      const value_type& _x, const value_type& _y, const value_type& _radius)
    : m_location(_x, _y), m_radius(_radius) {}
  ...
  WN_FORCE_INLINE value_type& at(const size_t _index) {
    WN_DEBUG_ASSERT_DESC(
        _index < 3, "Attempting to get element outside of bounds");
  ...
  WN_FORCE_INLINE value_type circumference() const {
    return (
        static_cast<value_type>(WN_PI) * static_cast<value_type>(2) * m_radius);
  }
  ...
  WN_FORCE_INLINE bool expand(
      const WNRectangle<value_type>& _box, const bool _anchor = false) {
  ...
  WN_FORCE_INLINE bool expand(
      const WNBounds2<value_type>& _bounds, const bool _anchor = false) {
    vector2<value_type> points[4];
  ...
  circle GetExpanded(
      const vector2<value_type>& _vector, bool _anchor = false) const;
  circle GetExpanded(
      const WNRectangle<value_type>& _rectangle, bool _anchor = false) const;
  circle GetExpanded(
      const WNBounds2<value_type>& _bounds, bool _anchor = false) const;
  circle GetExpanded(const circle& _circle, bool _anchor = false) const;

  WN_FORCE_INLINE circle extract_translated(
      const vector2<value_type>& _vector) const {
    circle circle(*this);
  ...
  WN_FORCE_INLINE circle extract_translated(
      const value_type& _x, const value_type& _y) const {
    circle circle(*this);
