...
public:
  ...
  WN_FORCE_INLINE sphere(const sphere& _sphere)
    : m_location(_sphere.m_location), m_radius(_sphere.m_radius) {}

  WN_FORCE_INLINE explicit sphere(const value_type* _numbers)
    : m_location(_numbers[0], _numbers[1], _numbers[2]),
      m_radius(_numbers[3]) {}

  WN_FORCE_INLINE explicit sphere(
      vector3<value_type>&& _location, value_type&& _radius)
    : m_location(std::move(_location)), m_radius(std::move(_radius)) {}

  WN_FORCE_INLINE explicit sphere(
      vector3<value_type>&& _location, const value_type& _radius)
    : m_location(std::move(_location)), m_radius(_radius) {}

  WN_FORCE_INLINE explicit sphere(
      const vector3<value_type>& _location, value_type&& _radius)
    : m_location(_location), m_radius(std::move(_radius)) {}

  WN_FORCE_INLINE explicit sphere(
      const vector3<value_type>& _location, const value_type& _radius)
    : m_location(_location), m_radius(_radius) {}

  WN_FORCE_INLINE explicit sphere(const value_type& _x, const value_type& _y,
      const value_type& _z, const value_type& _radius)
    : m_location(_x, _y, _z), m_radius(_radius) {}
  ...
  WN_FORCE_INLINE bool operator==(const sphere& _sphere) const {
  ...
  WN_FORCE_INLINE value_type& at(const size_t _index) {
    WN_DEBUG_ASSERT_DESC(
        _index < 4, "Attempting to get element outside of bounds");
  ...
  WN_FORCE_INLINE value_type volume() const {
    ...
    return (static_cast<value_type>(4) / static_cast<value_type>(3) *
            static_cast<value_type>(WN_PI) * radius_cubed);
  }

  WN_FORCE_INLINE value_type surface_area() const {
    const value_type radius_squared = m_radius * m_radius;

    return (static_cast<value_type>(4) * static_cast<value_type>(WN_PI) *
            radius_squared);
  }
  ...
  WN_FORCE_INLINE bool expand(
      const vector3<value_type>& _vector, const bool _anchor = false) {
  ...
  WN_FORCE_INLINE bool expand(
      const WNBox<value_type>& _box, const bool _anchor = false) {
  ...
  WN_FORCE_INLINE bool expand(
      const WNBounds3<value_type>& _bounds, const bool _anchor = false) {
    vector3<value_type> points[8];
  ...
  WN_FORCE_INLINE bool expand(
      const sphere& _sphere, const bool _anchor = false) {
    if (m_radius < static_cast<value_type>(0)) {
      if (!_anchor) {
        m_location = _sphere.m_location;
        m_radius = _sphere.m_radius;
      ...
      if (distance_squared >
          (m_radius + _sphere.m_radius) * (m_radius + _sphere.m_radius)) {
        const value_type distance = sqrt(distance_squared);

        if (!_anchor) {
          m_location += (_sphere.m_location - m_location) /
                        static_cast<value_type>(2) *
                        (static_cast<value_type>(1) -
                            m_radius / (distance + _sphere.m_radius));
          m_radius += ((distance + _sphere.m_radius) - m_radius) /
                      static_cast<value_type>(2);
        } else {
          m_radius = distance;
        }
  ...
  WN_FORCE_INLINE void translate(const vector3<value_type>& _vector) {
    m_location.translate(_vector);
  }

  WN_FORCE_INLINE void translate(
      const value_type& _x, const value_type& _y, const value_type& _z) {
  ...
  WN_FORCE_INLINE void translate_z(const value_type& _value) {
    m_location[2] += _value;
  }
  ...
  WN_FORCE_INLINE void assign(const value_type& _x, const value_type& _y,
      const value_type& _z, const value_type& _radius) {
  ...
  WN_FORCE_INLINE void set_location(vector3<value_type>&& _vector) {
    m_location = std::move(_vector);
  }

  WN_FORCE_INLINE void set_location(const vector3<value_type>& _vector) {
    m_location = _vector;
  }

  WN_FORCE_INLINE void set_location(
      const value_type& _x, const value_type& _y, const value_type& _z) {
  ...
  WN_FORCE_INLINE void set_location_z(const value_type& _value) {
    m_location[2] = _value;
  }
  ...
  WN_FORCE_INLINE sphere extract_expanded(
      const vector3<value_type>& _vector, const bool _anchor = false) const {
    sphere sphere(*this);
  ...
  WN_FORCE_INLINE sphere extract_expanded(
      const WNBox<value_type>& _box, const bool _anchor = false) const {
    sphere sphere(*this);
  ...
  WN_FORCE_INLINE sphere extract_expanded(
      const WNBounds3<value_type>& _bounds, const bool _anchor = false) const {
    sphere sphere(*this);
  ...
  WN_FORCE_INLINE sphere extract_expanded(
      const sphere& _sphere, const bool _anchor = false) const {
    sphere sphere(*this);

    return (sphere.expand(_sphere, _anchor), sphere);
  }

  WN_FORCE_INLINE sphere extract_translated(
      const vector3<value_type>& _vector) const {
    sphere sphere(*this);
  ...
  WN_FORCE_INLINE sphere extract_translated(
      const value_type& _x, const value_type& _y, const value_type& _z) const {
    sphere sphere(*this);
