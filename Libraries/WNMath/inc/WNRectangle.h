...
namespace wn {
    ...
    public:
        ...
        void Set(const vector3<type>& _location, vector3<type>& _extents, type _rotation);
        void Set(type _x1, type _y1, type _x2, type _y2, type _rotation);

        WNRectangle GetExpanded(type _amount) const;
        WNRectangle GetExpanded(const vector3<type>& _vector, bool _anchor = false) const;
        WNRectangle GetExpanded(const WNBounds2<type>& _bounds, bool _anchor = false) const;
        WNRectangle GetExpanded(const circle<type>& _circle, bool _anchor = false) const;
        WNRectangle GetExpanded(const WNRectangle& _rectangle, bool _anchor = false) const;
