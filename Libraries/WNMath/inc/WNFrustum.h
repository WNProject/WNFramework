...
template <typename type>
class WNFrustum {
  static_assert(wn::is_real<type>::value == true,
      "Frustums of non real number types don't make sense.  Please use one of "
      "the available real number types.");

public:
  explicit WNFrustum(const WNMatrix4<type>& _transform, type _near, type _far,
      type _aspect, type _fov, WNFieldOfView::WNHorizontal _horizontal);
  explicit WNFrustum(const WNMatrix4<type>& _transform, type _near, type _far,
      type _aspect, type _fov, WNFieldOfView::WNVertical _vertical);
  explicit WNFrustum(type _near, type _far, type _fov, type _aspect,
