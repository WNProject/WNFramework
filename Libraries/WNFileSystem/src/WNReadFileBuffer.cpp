...
wn_char* WNFileSystem::WNReadTextFileBuffer::ReserveBytes(const wn_size_t _numBytes, wn_size_t& _returnedBytes) {
    if (_numBytes + mBufferPosition > mBufferSize) {
        if (_numBytes > mBufferSize) {
            ...
            mSpareBuffer = wn::memory::realloc<wn_char>(mSpareBuffer, _numBytes);

            wn::memory::memcpy<wn_char>(mSpareBuffer, mCurrentBuffer + mBufferPosition, mBufferUsage - mBufferPosition);

            mCurrentBuffer = mSpareBuffer;
            mSpareBuffer = wn::memory::realloc<wn_char>(mCurrentBuffer, _numBytes);
