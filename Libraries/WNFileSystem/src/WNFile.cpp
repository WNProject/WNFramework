...
WNFileSystem::WNFile::~WNFile() {
    if (mFileBuffer) {
        ...
        wn::memory::free<wn_char>(mFileBuffer);
    }
...
wn_char* WNFileSystem::WNFile::GetDataBuffer() {
    ...
    mFileBuffer = wn::memory::malloc<wn_char>(mFileSize);
...
wn_char* WNFileSystem::WNFile::ResizeDataBuffer(wn_size_t _size) {
    if (mFileBuffer) {
        mFileBuffer = wn::memory::realloc<wn_char>(mFileBuffer, _size);
    } else {
        mFileBuffer = wn::memory::malloc<wn_char>(_size);
    }
