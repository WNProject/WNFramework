...
private:
  typedef std::iterator<std::bidirectional_iterator_tag, Element,
...
private:
  template <typename T>
  friend class contiguous_range;

  friend class contiguous_range_iterator<const Container, Container,
      const Element>;

  WN_FORCE_INLINE explicit contiguous_range_iterator(
      Container* _contiguous_range, const pointer _pointer)
    : m_owner(_contiguous_range), m_pointer(_pointer) {}
