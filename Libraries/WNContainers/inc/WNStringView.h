...
class string_view final {
 ...
 public:
  ...
  WN_FORCE_INLINE const_reference front() const { return (at(0)); }

  WN_FORCE_INLINE const_reference back() const { return (at(size() - 1)); }

  WN_FORCE_INLINE const_pointer data() const { return (m_range.data()); }

  // iterators

  WN_FORCE_INLINE const_iterator begin() const { return (cbegin()); }

  WN_FORCE_INLINE const_iterator cbegin() const { return (m_range.cbegin()); }

  WN_FORCE_INLINE const_iterator end() const { return (cend()); }

  WN_FORCE_INLINE const_iterator cend() const { return (m_range.cend()); }

  WN_FORCE_INLINE const_reverse_iterator rbegin() const { return (crbegin()); }

  WN_FORCE_INLINE const_reverse_iterator crbegin() const {
    return (const_reverse_iterator(cend()));
  }

  WN_FORCE_INLINE const_reverse_iterator rend() const { return (crend()); }
  ...
  WN_FORCE_INLINE size_type size() const { return (m_range.size()); }

  WN_FORCE_INLINE size_type length() const { return (size()); }

  WN_FORCE_INLINE size_type max_size() const { return (size()); }

  WN_FORCE_INLINE wn_bool empty() const { return (m_range.empty()); }
  ...
  WN_FORCE_INLINE string to_string(memory::allocator* _allocator) const {
    return (string(cbegin(), cend(), _allocator));
  }
  ...
  WN_FORCE_INLINE size_type find_last_of(const wn_char* s, const size_type pos,
                                         const size_type count) const {
...
struct hash<wn::containers::string_view> {
  size_t operator()(const wn::containers::string_view& string) const {
    return wn::memory::strnhash(string.data(), string.size());
  }
};
template <>
struct equal_to<wn::containers::string_view> {
  size_t operator()(const wn::containers::string_view& string,
                    const wn::containers::string_view& _other) const {
    ...
    return (wn::memory::strncmp(string.data(), _other.data(), string.size()) ==
            0);
  }
};
}  // namespace std
