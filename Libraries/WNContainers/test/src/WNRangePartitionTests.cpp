...
TEST_P(range_partition_tests, aligned) {
  ...
  for (size_t i = 0; i < 10; ++i) {
    tokens.emplace_back(l.get_interval(i + 1, static_cast<size_t>(1) << i));
    EXPECT_TRUE(tokens.back().is_valid());
...
TEST_P(range_partition_tests, small_aligned) {
  wn::testing::allocator m_allocator;
  using range_partition = wn::containers::range_partition<uint64_t>;
  range_partition l(&m_allocator, 16);

  auto tok0 = l.get_interval(1, 1);
  auto tok1 = l.get_interval(1, 4);
  auto tok2 = l.get_interval(1, 8);
