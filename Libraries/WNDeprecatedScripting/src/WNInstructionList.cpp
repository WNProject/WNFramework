...
WNInstructionList::WNInstructionList() : mReturns(false) {}

WNInstructionList::WNInstructionList(WNInstruction* inst) : mReturns(false) {
  mInstructions.PushBack(inst);
}

WNInstructionList::~WNInstructionList() {
  for (WNScriptLinkedList<WNInstruction>::WNScriptLinkedListNode* i =
           mInstructions.first;
       i != nullptr; i = i->next) {
...
eWNTypeError WNInstructionList::GenerateCode(WNCodeModule& _module,
  ...
  _module.GetScopedVariableList().PopScopeBlock(
      _module, !mReturns, _def, _compilationLog);
  return (ok);
}

bool WNInstructionList::Returns() {
  return (mReturns);
}
