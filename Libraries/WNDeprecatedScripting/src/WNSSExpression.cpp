...
WNSSExpression::WNSSExpression(
    WNSSType _type, WNExpression* _lhs, WNExpression* _rhs)
  : mSSType(_type), mLHS(_lhs), mRHS(_rhs) {}
...
eWNTypeError WNSSExpression::GenerateCode(WNCodeModule& _module,
  ...
  llvm::BasicBlock* endingBlock =
      llvm::BasicBlock::Create(llvm::getGlobalContext(), "", _def->mFunction);

  eWNTypeError err = ok;
  if (ok !=
      (err = _module.GetTypeManager().get_type_by_name("Bool", mScriptType))) {
  ...
  if (mRHS->GetType() != mScriptType) {
    const GenerateCastingOperation* castOp =
        _module.GetTypeManager().GetCastingOperation(
            mRHS->GetType(), mScriptType);
    if (!castOp) {
      _compilationLog.Log(WNLogging::eError, 0, "Cannot cast from ",
          mRHS->GetType()->mName, " to ", mScriptType->mName);
      LogLine(_compilationLog, WNLogging::eError);
      return (eWNInvalidCast);
    }
    if (ok != (err = castOp->Execute(builder, mRHS->GetValue(), rhsValue))) {
      _compilationLog.Log(WNLogging::eCritical, 0,
          "Error generating casting operation cast from ",
          mRHS->GetType()->mName, " to ", mScriptType->mName);
      LogLine(_compilationLog, WNLogging::eCritical);
      return (err);
    }
