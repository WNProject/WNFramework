...
eWNTypeError WNForInstruction::GenerateCode(WNCodeModule& _module,
  ...
  if (minitializer) {
    if (ok !=
        (err = minitializer->GenerateCode(_module, _def, _compilationLog))) {
      return (err);
    }
  }
  ...
  if (mCondition) {
    ...
    if (ok !=
      ...
      _compilationLog.Log(
          WNLogging::eCritical, 0, "Error cannot find bool type");
      LogLine(_compilationLog, WNLogging::eCritical);
      return (err);
    }
    if (ok !=
        (err = mCondition->GenerateCode(_module, _def, _compilationLog))) {
      return (err);
    }
    ...
    if (mCondition->GetType() != boolType) {
      const GenerateCastingOperation* castOp =
          _module.GetTypeManager().GetCastingOperation(
              mCondition->GetType(), boolType);
      if (!castOp) {
        _compilationLog.Log(WNLogging::eError, 0,
            "Error cannot cast to Bool from", mCondition->GetType()->mName);
      ...
      if (ok != (err = castOp->Execute(builder, condValue, condValue))) {
        _compilationLog.Log(WNLogging::eCritical, 0,
            "Error creating cast to Bool from", mCondition->GetType()->mName);
        LogLine(_compilationLog, WNLogging::eCritical);
        return (err);
      }
    }
  ...
  _module.GetScopedVariableList().PopScopeBlock(
      _module, mCondition != NULL, _def, _compilationLog);
  return (ok);
}
