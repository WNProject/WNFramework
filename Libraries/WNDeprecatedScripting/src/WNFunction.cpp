...
eWNTypeError WNFunction::GenerateCode(WNCodeModule& _module,
    WNScriptType owningType, WNLogging::WNLog& _compilationLog) {
  if (mIsOverride && mIsVirtual) {
    _compilationLog.Log(WNLogging::eError, 0, "Error: Function ",
        mDeclaration->GetName(), " can not be both virtual and override");
  ...
  if (owningType && owningType->mName) {
    ...
    WN_DEBUG_ASSERT(
        wn::memory::strlen(funcName) == wn::memory::strlen(owningType->mName));
    wn::memory::strcat(funcName, "$");
  }
  wn::memory::strncat(funcName, mDeclaration->GetName(), 1024);

  if (ok != (err = mDeclaration->GetType(
                 _module.GetTypeManager(), returnType, _compilationLog))) {
  ...
  if (mParameters) {
    if (ok != (err = mParameters->GetTypeList(_module.GetTypeManager(),
                   parameterTypes, _compilationLog))) {
  ...
  WNFunctionDefinition* def =
      _module.GetFunctionDefinition(funcName, parameterTypes);
  if (!def) {
    _compilationLog.Log(
        WNLogging::eError, 0, "Function Definition does not exist: ", funcName);
    LogLine(_compilationLog, WNLogging::eError);
    return (error);
  }
  llvm::BasicBlock* bb =
  ...
  if (owningType) {
    ...
    WNScriptVariable* var =
        WN_SCRIPTNODE_NEW(WNScriptVariable(owningType, "this", mThis, true));
    if (ok != (err = _module.GetScopedVariableList().PushVariable(var))) {
      _compilationLog.Log(
          WNLogging::eError, 0, "Variable already exists ", "this");
      LogLine(_compilationLog, WNLogging::eError);
      return (err);
    }
    ...
    for (size_t i = 0; i < owningType->mStructTypes.size(); ++i) {
      ...
      if (ok != (err = _module.GetScopedVariableList().PushVariable(
                     new WNScriptVariable(st.mType, st.mName, val, true)))) {
        _compilationLog.Log(
            WNLogging::eError, 0, "Variable already exists ", st.mName);
        LogLine(_compilationLog, WNLogging::eError);
        return (err);
      }
    }
  ...
  if (mParameters) {
    ...
    if (ok != (err = mParameters->GetDeclarations(declarations))) {
      _compilationLog.Log(
          WNLogging::eError, 0, "Cannot find declarations for parameters ");
      LogLine(_compilationLog, WNLogging::eError);
      return (err);
    }

    WNScriptLinkedList<WNDeclaration>::WNScriptLinkedListNode* iter =
        declarations->first;
    for (; iter != nullptr && fIter != def->mFunction->arg_end();
         iter = iter->next, ++fIter) {
      fIter->setName(iter->value->GetName());
      WNScriptType t = nullptr;
      if (ok != (err = iter->value->GetType(
                     _module.GetTypeManager(), t, _compilationLog))) {
        _compilationLog.Log(WNLogging::eError, 0,
            "Cannot find type for declaration: ", iter->value->GetName());
        LogLine(_compilationLog, WNLogging::eError);
        return (err);
      }
      ...
      if (!alloc) {
        _compilationLog.Log(WNLogging::eError, 0,
            "Cannot allocate variable of type: ", t->mName);
        LogLine(_compilationLog, WNLogging::eError);
        return (eWNCannotCreateType);
      }
      llvm::Value* allocLoc = nullptr;
      if (ok != (err = alloc->Execute(
                     _module, t, iter->value->GetName(), false, allocLoc))) {
        _compilationLog.Log(WNLogging::eCritical, 0,
            "Error allocating a variable of type: ", t->mName);
        LogLine(_compilationLog, WNLogging::eCritical);
        return (err);
      }
      const GenerateAssignment* assign =
          _module.GetTypeManager().GetAssignmentOperation(t, AT_EQ);
      if (!assign) {
        _compilationLog.Log(WNLogging::eError, 0,
            "Cannot assign a variable of type: ", t->mName);
        LogLine(_compilationLog, WNLogging::eError);
      }
      if (ok != (err = assign->Execute(_module, true, t, fIter, allocLoc, def,
                     _compilationLog))) {
        _compilationLog.Log(WNLogging::eCritical, 0,
            "Error assigning a variable of type: ", t->mName);
        LogLine(_compilationLog, WNLogging::eCritical);
        return (err);
      }
      WNScriptVariable* var = WN_SCRIPTNODE_NEW(
          WNScriptVariable(t, iter->value->GetName(), allocLoc));
      if (ok != (err = _module.GetScopedVariableList().PushVariable(var))) {
        _compilationLog.Log(WNLogging::eError, 0, "Variable already exists ",
            iter->value->GetName());
        LogLine(_compilationLog, WNLogging::eError);
        return (err);
      }
    ...
    if (fIter != def->mFunction->arg_end() && iter != nullptr) {
      _compilationLog.Log(
          WNLogging::eError, 0, "Invalid number of parameters to function");
      LogLine(_compilationLog, WNLogging::eError);
      return (error);
    }
  }
  ...
  if (!mBody->Returns()) {
    if (ok != (err = _module.GetScopedVariableList().GenerateReturn(
                   _module, def, _compilationLog))) {
      _compilationLog.Log(
          WNLogging::eCritical, 0, "Cannot create proper return");
      LogLine(_compilationLog, WNLogging::eCritical);
      return (err);
    }
    if (ok != (err = GenerateVOIDReturn(_module, def))) {
      _compilationLog.Log(WNLogging::eError, 0,
          "Trying to create void return for non-void function");
      LogLine(_compilationLog, WNLogging::eError);
      return (err);
    }
  }
...
eWNTypeError WNFunction::ExportFunctions(WNCodeModule& _module,
    WNScriptType owningType, std::vector<WNFunctionDefinition*>& _definitions,
  ...
  if (mIsOverride && !owningType) {
    _compilationLog.Log(WNLogging::eError, 0,
        "Error it is invalid to have an override function that is not in a "
        "class");
    LogLine(_compilationLog, WNLogging::eError);
  }
  ...
  if (ok != (err = _module.GenerateFunctionDefinition(funcName, parameterTypes,
                 returnType, def, owningType, mIsVirtual || mIsOverride))) {
  ...
  llvm::FunctionType* fType =
      llvm::FunctionType::get(returnType->mLLVMType, llvmTypes, false);
...
eWNTypeError WNFunction::GenerateHeader(WNCodeModule& _module,
    WNScriptType owningType, WNLogging::WNLog& _compilationLog) {
  eWNTypeError err = ok;
  WNScriptType returnType;
  char funcName[2048];
  ...
  if (ok != (err = _module.AddFunctionDefinition(funcName, parameterTypes,
                 returnType, def, equivDef, owningType))) {
  ...
  llvm::Function* func = llvm::Function::Create(
      fType, llvm::GlobalValue::ExternalLinkage, funcName, _module.GetModule());
  def->mFunction = func;
  def->mFunctionType = fType;
