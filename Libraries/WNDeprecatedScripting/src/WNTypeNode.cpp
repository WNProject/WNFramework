...
WNTypeNode::WNTypeNode(const char* _customType)
  : mType(SC_CUSTOM), mNumArrayLevels(0) {
...
eWNTypeError WNTypeNode::GetType(WNTypeManager& _manager,
    WNScriptType& _outType, WNLogging::WNLog& _compilationLog) const {
  eWNTypeError err = ok;
  if (ok != (err = _manager.get_type_by_name(mCustomType, _outType))) {
    _compilationLog.Log(
        WNLogging::eError, 0, "Could not find type ", mCustomType);
