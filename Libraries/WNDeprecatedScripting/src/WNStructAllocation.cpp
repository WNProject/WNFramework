...
WNStructAllocation::WNStructAllocation()
  : mType(nullptr), mCopyinitializer(nullptr) {}
...
eWNTypeError WNStructAllocation::GenerateCode(WNCodeModule& _module,
  ...
  if (ok != (err = mType->GetType(
                 _module.GetTypeManager(), structType, _compilationLog))) {
    return (err);
  }
  if (!structType->mLLVMStructType) {
    _compilationLog.Log(
        WNLogging::eError, 0, structType->mName, " is not a valid Struct type");
    ...
    if (mCopyinitializer->GetType() != structType) {
      const GenerateCastingOperation* op =
          _module.GetTypeManager().GetCastingOperation(
              mCopyinitializer->GetType(), structType);
      if (!op) {
        _compilationLog.Log(WNLogging::eError, 0,
            "Could not copy construct object of type ", structType->mName,
            " with object of type ", mCopyinitializer->GetType()->mName);
        LogLine(_compilationLog, WNLogging::eError);
        return (eWNInvalidCast);
      }
      if (ok != (err = op->Execute(_module.GetBuilder(),
                     mCopyinitializer->GetValue(), copyValue))) {
        _compilationLog.Log(WNLogging::eCritical, 0, "Error copy constructing ",
            structType->mName);
  ...
  if (!mCopyinitializer) {
    const GenerateConstruction* construction =
        _module.GetTypeManager().GetConstructionOperation(structType);
    if (!construction) {
      _compilationLog.Log(
          WNLogging::eCritical, 0, "No Constructor for ", structType->mName);
      LogLine(_compilationLog, WNLogging::eCritical);
    }
    if (ok != (err = construction->Execute(_module, outValue, structType,
                   nullptr, _def, _compilationLog))) {
      _compilationLog.Log(
          WNLogging::eError, 0, "Cannot allocate Struct: ", structType->mName);
      LogLine(_compilationLog, WNLogging::eError);
      return (err);
    }
  } else {
    const GenerateCopyConstruction* construction =
        _module.GetTypeManager().GetCopyConstructionOperation(structType);
    if (!construction) {
      _compilationLog.Log(
          WNLogging::eCritical, 0, "No Constructor for ", structType->mName);
      LogLine(_compilationLog, WNLogging::eCritical);
