...
namespace WNScripting {
    ...
    eWNTypeError WNBuiltIninitializer::InitializeScriptingCasts(WNScriptingEngine*, WNTypeManager& _manager) {
        eWNTypeError err = ok;
        WNScriptType nullType; if((err = _manager.get_type_by_name("-Null", nullType)) != ok) { return err; }
        _manager.RegisterAllocationOperator(nullType, wn::memory::construct<GenerateDefaultAllocation>());

        WNScriptType floatType;  if((err = _manager.get_type_by_name("Float", floatType)) != ok) { return err; }
        WNScriptType intType; if((err = _manager.get_type_by_name("Int", intType)) != ok) { return err; }
        WNScriptType charType;  if((err = _manager.get_type_by_name("Char", charType)) != ok) { return err; }
        WNScriptType boolType; if((err = _manager.get_type_by_name("Bool", boolType)) != ok) { return err; }
        WNScriptType sizeTType; if((err = _manager.get_type_by_name("-SizeT", sizeTType)) != ok) { return err; }
