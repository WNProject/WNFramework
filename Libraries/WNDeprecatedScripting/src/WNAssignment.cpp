...
WNAssignment::WNAssignment(WNLValue* _lValue)
  : mLValue(_lValue), mAssignType(AT_MAX), mAssignExpression(nullptr) {}
...
eWNTypeError WNAssignment::GenerateCode(WNCodeModule& _module,
  ...
  if (!mAssignExpression) {
    if (mLValue->RequiredUse()) {
      _compilationLog.Log(WNLogging::eError, 0, mLValue->GetValueType()->mName,
          "must be assigned to");
  ...
  if (!mLValue->GetValueLocation()) {
    _compilationLog.Log(WNLogging::eError, 0, mLValue->GetValueType()->mName,
        "is not a valid assignee");
    LogLine(_compilationLog, WNLogging::eError);
    return (eWNBadType);
  }
  const GenerateAssignment* assign =
      _module.GetTypeManager().GetAssignmentOperation(
          mLValue->GetValueType(), mAssignType);
  if (!assign) {
    _compilationLog.Log(WNLogging::eError, 0, "Cannot generate assignment for ",
        mLValue->GetValueType()->mName);
    LogLine(_compilationLog, WNLogging::eError);
    return (eWNBadType);
  }
  ...
  if (ok !=
      (err = mAssignExpression->GenerateCode(_module, _def, _compilationLog))) {
    return (err);
  }
  ...
  if (mAssignExpression->GetType() != type) {
    const GenerateCastingOperation* op =
        _module.GetTypeManager().GetCastingOperation(
            mAssignExpression->GetType(), mLValue->GetValueType());
    if (op == nullptr) {
      _compilationLog.Log(WNLogging::eError, 0, "Cannot cast from ",
          mAssignExpression->GetType()->mName, " to ",
          mLValue->GetValueType()->mName);
      LogLine(_compilationLog, WNLogging::eError);
    ...
    if (ok != (err = op->Execute(
                   _module.GetBuilder(), mAssignExpression->GetValue(), val))) {
      _compilationLog.Log(WNLogging::eCritical, 0,
          "Error generating cast from ", mAssignExpression->GetType()->mName,
          " to ", mLValue->GetValueType()->mName);
      LogLine(_compilationLog, WNLogging::eCritical);
      return (ok);
    }
  }
  if (ok != (err = assign->Execute(_module, false, type, val,
                 mLValue->GetValueLocation(), _def, _compilationLog))) {
