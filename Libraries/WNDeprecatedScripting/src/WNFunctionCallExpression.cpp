...
WNFunctionCallExpr::WNFunctionCallExpr() : mArgs(nullptr) {}

WNFunctionCallExpr::WNFunctionCallExpr(WNArgList* _list) : mArgs(_list) {}
...
eWNTypeError WNFunctionCallExpr::GenerateCode(WNCodeModule& _module,
  ...
  if (ok !=
      (err = _module.GetTypeManager().get_type_by_name("Void", voidType))) {
  ...
  if ((testValue == nullptr || testType == nullptr) &&
      _def->mThisType != nullptr) {
    testType = _def->mThisType;
    const WNScriptVariable* var =
        _module.GetScopedVariableList().GetVariable("this");
    if (!var) {
      _compilationLog.Log(
          WNLogging::eCritical, 0, "Error cannot find \'this\'");
      LogLine(_compilationLog, WNLogging::eCritical);
      return (eWNInvalidOperation);
    }
  ...
  WNScriptLinkedList<WNFunctionExpression>& localExpressionList =
      mArgs ? mArgs->GetExpressions() : staticList;
  std::vector<FunctionParam> scriptTypes;
  for (WNScriptLinkedList<WNFunctionExpression>::WNScriptLinkedListNode* i =
           localExpressionList.first;
       i != nullptr; i = i->next) {
    if (ok !=
        (err = i->value->expr->GenerateCode(_module, _def, _compilationLog))) {
      return (err);
    }
    if (i->value->expr->RequiredUse()) {
      _compilationLog.Log(WNLogging::eError, 0,
          "You must assign parameter before passing to subsequent function");
    ...
    if (i->value->handOwnership) {  // looks like Int a = x(<== y);
      ...
      if (!(i->value->expr->GetType()->mLLVMStructType &&
              i->value->expr->GetValueLocation())) {
        // we are trying to give away ownership of a non-obect or a
        // non-referencable object
        _compilationLog.Log(WNLogging::eError, 0,
            "Trying to give away ownership of invalid object");
        LogLine(_compilationLog, WNLogging::eError);
        return (error);
      }
      ...
      builder->CreateStore(llvm::ConstantInt::get(ptrType, 0),
          builder->CreatePointerCast(ownLocation, ptrType->getPointerTo(0)));
    }
  }

  if (testType) {
    FunctionParam param = {testValue, testType};
    if (ok != (err = GenerateRecursiveThisFunctionCall(_module, _def,
                   reinterpret_cast<char*>(mBaseExpression->GetValue()),
                   scriptTypes, mScriptType, mValue, param, _compilationLog))) {
      if (forceThis) {  // we made an explicit function call (a.doSomething()),
        // therefore we CANNOT remove the "this" parameter
        _compilationLog.Log(
            WNLogging::eError, 0, "Cannot create function call");
        LogLine(_compilationLog, WNLogging::eError);
        return (err);
      }
    } else {
  ...
  if (ok != (err = GenerateFunctionCall(_module, _def,
                 reinterpret_cast<char*>(mBaseExpression->GetValue()),
                 scriptTypes, true, mScriptType, mValue, _compilationLog))) {
