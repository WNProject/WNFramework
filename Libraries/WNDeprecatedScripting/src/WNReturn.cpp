...
WNReturn::WNReturn(WNExpression* _expr, bool _changeOwnership)
  : mExpression(_expr), mChangeOwnership(_changeOwnership) {}

WNReturn::WNReturn() : mExpression(nullptr) {}
...
eWNTypeError WNReturn::GenerateCode(WNCodeModule& _module,
  ...
  if (mExpression) {
    ...
    if (ok !=
        (err = mExpression->GenerateCode(_module, _def, _compilationLog))) {
      return (err);
    }
    ...
    if (_def->mReturn != mExpression->GetType()) {
      const GenerateCastingOperation* castOp =
          _module.GetTypeManager().GetCastingOperation(
              mExpression->GetType(), _def->mReturn);
      if (!castOp) {
        _compilationLog.Log(WNLogging::eError, 0, "Cannot cast ",
            mExpression->GetType(), " to ", _def->mReturn->mName);
        LogLine(_compilationLog, WNLogging::eError);
        return (eWNInvalidCast);
      }
      if (ok != (err = castOp->Execute(
                     _module.GetBuilder(), mExpression->GetValue(), v))) {
        _compilationLog.Log(WNLogging::eCritical, 0,
            "Error generating casting operation ",
            mExpression->GetType()->mName, " to ", _def->mReturn->mName);
    ...
    if (mChangeOwnership) {
      if (!((_def->mReturn->mLLVMStructType || _def->mReturn->mArrayType) &&
              mExpression->GetValueLocation())) {
        _compilationLog.Log(WNLogging::eCritical, 0, "Cannot change of type ",
            mExpression->GetType()->mName);
        LogLine(_compilationLog, WNLogging::eCritical);
        return (error);
      }
      ...
      llvm::Value* spLoc =
          builder->CreateLoad(builder->CreateGEP(structLoc, GepArray));
      ...
      llvm::Value* cmp =
          builder->CreateICmpEQ(builder->CreatePtrToInt(tempValue, ptrType),
              builder->CreatePtrToInt(structLoc, ptrType));

      builder->CreateCondBr(cmp, destroyBlock, endBlock);
      builder->SetInsertPoint(destroyBlock);
      builder->CreateStore(llvm::ConstantInt::get(ptrType, 0),
          builder->CreatePointerCast(valueLoc, ptrType->getPointerTo(0)));
      GepArray.clear();
      GepArray.push_back(llvm::ConstantInt::get(
          llvm::IntegerType::getInt32Ty(llvm::getGlobalContext()), 0));
      GepArray.push_back(llvm::ConstantInt::get(
    ...
    } else if ((_def->mReturn->mLLVMStructType || _def->mReturn->mArrayType) &&
               mExpression->GetValueLocation()) {
      llvm::Value* structLoc = mExpression->GetValueLocation();
      std::vector<llvm::Value*> GepArray;
      GepArray.push_back(llvm::ConstantInt::get(
    ...
    if ((_def->mReturn->mLLVMStructType || _def->mReturn->mArrayType) &&
      ...
      GepArray.push_back(llvm::ConstantInt::get(
          llvm::IntegerType::getInt32Ty(llvm::getGlobalContext()), 2));
      llvm::Value* countLocation = builder->CreateGEP(spLoc, GepArray);
      builder->CreateAtomicRMW(llvm::AtomicRMWInst::Sub, countLocation,
          llvm::ConstantInt::get(ptrType, 1), llvm::Monotonic);
    }
    ...
    if (ok != (err = GenerateVOIDReturn(_module, _def))) {
      _compilationLog.Log(
          WNLogging::eCritical, 0, "Error generating void return");
      LogLine(_compilationLog, WNLogging::eCritical);
      return (err);
    }
  }
