...
eWNTypeError WNElseIf::GenerateCode(
    WNCodeModule&, const WNFunctionDefinition*, WNLogging::WNLog&) {
  return (error);  // The code for the Elseif is actually generated in
                   // WNIFInstruction
}
...
WNIFInstruction::WNIFInstruction(WNExpression* _cond, WNInstructionList* _body)
  : mCondition(_cond), mBody(_body), mElse(nullptr) {}
...
WNIFInstruction::~WNIFInstruction() {
  ...
  for (WNScriptLinkedList<WNElseIf>::WNScriptLinkedListNode* i =
           mElseIfNodes.first;
       i != nullptr; i = i->next) {
...
eWNTypeError WNIFInstruction::GenerateCode(WNCodeModule& _module,
  ...
  for (WNScriptLinkedList<WNElseIf>::WNScriptLinkedListNode* i =
       ...
       i != nullptr; i = i->next) {
    if (ok !=
        (err = curCondition->GenerateCode(_module, _def, _compilationLog))) {
      return (err);
    }
    if (curCondition->GetType() != boolType) {
      _compilationLog.Log(
          WNLogging::eError, 0, "Condition must be of boolean type");
    ...
    if (!returns) {
      ...
      if (!endBlock) {
        endBlock = llvm::BasicBlock::Create(
            llvm::getGlobalContext(), "", _def->mFunction);
      }
