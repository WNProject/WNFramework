...
WNConstantExpression::WNConstantExpression(
    WNScriptTypeName _type, const char* _text)
  : mTypeName(_type), mText(nullptr) {
...
eWNTypeError WNConstantExpression::GenerateCode(WNCodeModule& _module,
    const WNFunctionDefinition*, WNLogging::WNLog& _compilationLog) {
  ...
  if (ok !=
      _module.GetTypeManager().get_type_by_name(
          WNScriptTypeNames[mTypeName], mScriptType)) {
    _compilationLog.Log(WNLogging::eError, 0,
        "Trying to create constant of invalid type: ",
        WNScriptTypeNames[mTypeName]);
  ...
  const GenerateConstantOperation* op =
      _module.GetTypeManager().GetConstantOperation(mScriptType);
  if (!op) {
    _compilationLog.Log(WNLogging::eError, 0,
        "No constant generation possible for: ", WNScriptTypeNames[mTypeName]);
    LogLine(_compilationLog, WNLogging::eError);
    return (eWNBadType);
  }
