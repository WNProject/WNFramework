...
WNWhileInstruction::WNWhileInstruction(
    WNExpression* _cond, WNInstructionList* _body)
...
eWNTypeError WNWhileInstruction::GenerateCode(WNCodeModule& _module,
  ...
  if (mCondition->GetType() != boolType) {
    _compilationLog.Log(
        WNLogging::eError, 0, "Conditional type must be boolean ");
