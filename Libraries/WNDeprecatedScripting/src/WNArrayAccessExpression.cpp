...
eWNTypeError WNArrayAccessExpr::GenerateCode(WNCodeModule& _module,
  ...
  if (ok !=
      (err = mArrayAccess->GenerateCode(_module, _def, _compilationLog))) {
    return (err);
  }
  ...
  if (intType != mArrayAccess->GetType()) {
    const GenerateCastingOperation* castOp =
        _module.GetTypeManager().GetCastingOperation(
            mArrayAccess->GetType(), intType);
    if (!castOp) {
      _compilationLog.Log(WNLogging::eError, 0, "Cannot cast from ",
          mArrayAccess->GetType()->mName, " to ", intType->mName);
  ...
  const GenerateArrayAccessOperation* arrayOp =
      _module.GetTypeManager().GetArrayAccessOperation(
          mBaseExpression->GetType(), intType);
  if (!arrayOp) {
    _compilationLog.Log(WNLogging::eError, 0,
        "No valid array access operator exists for ",
        mBaseExpression->GetType()->mName);
    LogLine(_compilationLog, WNLogging::eError);
    return (eWNInvalidOperation);
  }

  if (ok != (err = arrayOp->Execute(_module, mBaseExpression->GetValue(), rhs,
                 mValue, mValueLocation, _def))) {
    _compilationLog.Log(
        WNLogging::eCritical, 0, "Error generating array access operation ");
    LogLine(_compilationLog, WNLogging::eCritical);
    return (err);
  }
