...
namespace WNScripting {
    ...
    public:
        ...
        virtual eWNTypeError GetExistingFunctionPointer(const char* _file, const char* _functionName, WNScriptType& _retParam, const std::vector<WNScriptType>& _params, void*& _ptr) const;
    ...
    private:
        static void BufferFlushCallback(void* _context, const char* buffer, size_t bufferLength, const std::vector<WNLogging::WNLogColorElement>& colors);
        ...
        static void* CompileLazyFile(void* _scriptingEngine, void* _requestedFunction);
        eWNTypeError compile_file(const char* _file, WNCodeModule*& _outModule);
        WNCodeModule* GetCompiledModule(const char* _file) const;
        eWNTypeError PreprocessFile(const char* _file, WNCodeModule*& _module, WNPreprocessedFile*& _outFile, std::list<WNStruct*>& _toBeLinked);
