...
#pragma message(                                                               \
    "TODO(awoloszyn): Figure out what this is actually for, you cannot remember.")
...
public:
#pragma message(                                                               \
    "TODO(awoloszyn): binary_expression should create an implicit cast expression as a child.")
...
public:
  ...
  instruction_list(instruction* inst) {
...
public:
  ...
  void add_function(function* _func) {
    m_struct_functions.emplace_back(_func);
  }
...
public:
  decl_list(declaration* _decl) {
    m_declarations.emplace_back(_decl);
  }
...
private:
  ...
  wn::memory::unique_ptr<instruction_list> m_body;
  wn::containers::deque<wn::memory::unique_ptr<else_if_instruction>>
      m_else_if_nodes;
};
...
public:
  ...
  void add_initializer(instruction* _init) {
    m_initializer.reset(_init);
  }
  void add_condition(expression* _cond) {
    m_condition.reset(_cond);
  }
  void add_post_op(instruction* _inst) {
    m_post_op.reset(_inst);
  }
...
public:
  ...
  void add_function(function* _node) {
    m_functions.emplace_back(_node);
  }

  void add_struct(struct_definition* _node) {
    m_structs.emplace_back(_node);
  }

  void add_include(wn_char* _node) {
    m_includes.push_back(_node);
  }
