...
WN_INLINE void FreeArray(void* _array) {
  ...
  if (sIp) {
    if (sIp->owner == _array && 0 != sIp->structLoc) {
      ...
      WNScriptType type = *reinterpret_cast<WNScriptType*>(
          (reinterpret_cast<size_t>(sIp->structLoc) + sizeof(size_t)));
      WNScriptType nType = type->mArrayType;
      if (nType->mStoredVTable || nType->mArrayType) {
        if (nType->mStoredVTable) {
          for (size_t i = 0; i < curSize; ++i) {
            FreeStruct(reinterpret_cast<char*>(sIp->structLoc) +
                       sizeof(size_t) * 2 + i * nType->mTypeSize);
          }
        } else {
          for (size_t i = 0; i < curSize; ++i) {
            FreeArray(reinterpret_cast<char*>(sIp->structLoc) +
                      sizeof(size_t) * 2 + i * nType->mTypeSize);
          }
        }
      }
...
WN_INLINE void Resize(void* _array, size_t _arraySize) {
  ...
  if (nType->mStoredVTable || nType->mArrayType) {
    if (nType->mStoredVTable) {
      ...
      for (size_t i = _arraySize; i < curSize; ++i) {
        FreeArray(reinterpret_cast<char*>(sIp->structLoc) + sizeof(size_t) * 2 +
                  i * nType->mTypeSize);
      }
    }
    for (size_t i = 0; i < ((curSize < _arraySize) ? curSize : _arraySize);
         ++i) {
      StructInternalType** sp = reinterpret_cast<StructInternalType**>(
          reinterpret_cast<char*>(sIp->structLoc) + sizeof(size_t) * 2 +
          i * nType->mTypeSize);
      if ((*sp)->owner == sp) {
        (*sp)->owner = 0;
      }
    }
    sIp->structLoc = wn::memory::realloc(
        sIp->structLoc, nType->mTypeSize * _arraySize + sizeof(size_t) * 2);
    for (size_t i = 0; i < ((curSize < _arraySize) ? curSize : _arraySize);
         ++i) {
      StructInternalType** sp = reinterpret_cast<StructInternalType**>(
          reinterpret_cast<char*>(sIp->structLoc) + sizeof(size_t) * 2 +
    ...
    for (size_t i = curSize; i < _arraySize; ++i) {
      StructInternalType* sp = reinterpret_cast<StructInternalType*>(
          malloc(sizeof(StructInternalType)));
      *reinterpret_cast<StructInternalType**>(
          reinterpret_cast<char*>(sIp->structLoc) + sizeof(size_t) * 2 +
          i * nType->mTypeSize) = sp;
      sp->owner = reinterpret_cast<StructInternalType**>(
          reinterpret_cast<char*>(sIp->structLoc) + sizeof(size_t) * 2 +
          i * nType->mTypeSize);
      sp->refCount = 1;
    ...
    if (_arraySize > curSize) {
      wn::memory::memset(reinterpret_cast<char*>(sIp->structLoc) +
                             sizeof(size_t) * 2 + curSize * nType->mTypeSize,
          0x00, (_arraySize - curSize) * nType->mTypeSize);
    }
  }
...
public:
  ...
  WNScriptingObjectRef<T, T_Parent>* DetachOut() {
    WNScriptingObject<T, T_Parent>* ptr =
        reinterpret_cast<WNScriptingObject<T, T_Parent>*>(refData);
    refData->refCount--;
    refData = nullptr;
    return (ptr);
  }
  WNScriptingObject<T, T_Parent>& operator=(
      WNScriptingObject<T, T_Parent>&& _other) {
    Destroy();
    refData = _other.refData;
  ...
  operator typename ParentType::ParentType::ParentType::ParentType::
      ParentType() {
    return (
        ParentType::ParentType::ParentType::ParentType::ParentType(refData));
  }
  operator typename ParentType::ParentType::ParentType::ParentType::ParentType::
      ParentType() {
    return (
        ParentType::ParentType::ParentType::ParentType::ParentType::ParentType(
            refData));
  }
  operator typename ParentType::ParentType::ParentType::ParentType::ParentType::
      ParentType::ParentType() {
    return (ParentType::ParentType::ParentType::ParentType::ParentType::
            ParentType::ParentType(refData));
  }
  operator typename ParentType::ParentType::ParentType::ParentType::ParentType::
      ParentType::ParentType::ParentType() {
    return (ParentType::ParentType::ParentType::ParentType::ParentType::
            ParentType::ParentType::ParentType(refData));
  }
  operator typename ParentType::ParentType::ParentType::ParentType::ParentType::
      ParentType::ParentType::ParentType::ParentType() {
    return (ParentType::ParentType::ParentType::ParentType::ParentType::
            ParentType::ParentType::ParentType::ParentType(refData));
  }
  operator typename ParentType::ParentType::ParentType::ParentType::ParentType::
      ParentType::ParentType::ParentType::ParentType::ParentType() {
    return (
        ParentType::ParentType::ParentType::ParentType::ParentType::ParentType::
            ParentType::ParentType::ParentType::ParentType(refData));
  }
  ...
  template <typename TR, typename R, typename T1, typename T2, typename T3,
      typename T4, typename T5, typename T6>
  friend class WNScripting::WNMemberFunctionPointer;
};
enum eResolutionState { eUnresolved, eResolved, eFailedResolution };
...
public:
  ...
  T& GetValue(const PT& x) {
    void* v = reinterpret_cast<void*>(
        reinterpret_cast<size_t>(x.refData->structLoc) + mOffset);
    return (*reinterpret_cast<T*>(v));
  }
  WNMemberVariablePointer(const char* _name) : mEngine(nullptr) {
    mVariableName = wn::memory::strndup(_name, 1024);
  }
  void SetEngine(WNScriptingEngine* _engine) {
    mEngine = _engine;
  }
  WNMemberVariablePointer(const char* _name, WNScriptingEngine* _engine)
    : mOffset(-1) {
    mEngine = _engine;
    mVariableName = wn::memory::strndup(_name, 1024);
...
private:
  void TryToResolve() {
    ...
    if (mVariableName) {
      WNScriptType type =
          mEngine->get_type_by_name(TypeMapping<PT>::get_type_name());

      if (type != 0) {
        std::vector<WNContainedStructType>::iterator i =
            std::find_if(type->mStructTypes.begin(), type->mStructTypes.end(),
                [this](const WNContainedStructType& sType) {
                  return (wn::memory::strcmp(mVariableName, sType.mName) == 0);
                });
        ...
        if (i->mType !=
            mEngine->get_type_by_name(TypeMapping<T>::get_type_name())) {
          mResolution = eFailedResolution;
          return;
        }
...
public:
  ...
  WNScriptingArrayRef<T>* DetachOut() {
    if (refData) {
      WNScriptingArrayRef<T>* ptr =
          reinterpret_cast<WNScriptingArrayRef<T>*>(refData);
      refData->refCount--;
      refData = nullptr;
      return (ptr);
    }
  ...
  WNScriptType GetType() {
    ...
    WNScriptType type = *reinterpret_cast<WNScriptType*>(
        (reinterpret_cast<size_t>(refData->structLoc) + sizeof(size_t)));
    return (type);
  }

  T& operator[](size_t i) {
    void* v =
        reinterpret_cast<void*>(reinterpret_cast<size_t>(refData->structLoc) +
                                2 * sizeof(size_t) + (i * sizeof(T)));
    return (*reinterpret_cast<T*>(v));
  }
  size_t GetLength() {
    if (refData) {
...
struct TypeMapping<WNScriptingArray<T>> {
  ...
  static const char* get_type_name() {
    return TypeMapping<T>::get_type_name();
  }
  static WNScriptType GetModifiedType(WNScriptType t, WNScriptingEngine* e) {
    return e->get_array_of(TypeMapping<T>::GetModifiedType(t, e));
  }
};
...
public:
  ...
  ScriptPointerRef<T, T_Parent>* DetachOut() {
    ScriptPointerRef<T, T_Parent>* ptr =
        reinterpret_cast<ScriptPointerRef<T, T_Parent>*>(refData);
    refData->refCount--;
    refData = nullptr;
    return (ptr);
  }
...
#define DEFINE_TEMPLATE_MAPPING(_type, _name)                                  \
  ...
  struct TypeMapping<_type> {                                                  \
    ...
    static WNScriptType GetModifiedType(WNScriptType t, WNScriptingEngine*) {  \
      return t;                                                                \
    }                                                                          \
  };                                                                           \
  }
...
public:
  ...
  R operator()() {
    static_assert(IsDummy<T1>::val == 1, WN_STRINGERIZE(IsDummy<T1>::val == 1));
    return (OutCaster<R>::ConvertOut(
        reinterpret_cast<typename OutCaster<R>::InType (*)()>(callPt)()));
  }
  R operator()(const T1& t1) {
    static_assert(IsDummy<T1>::val == 0 && IsDummy<T2>::val == 1,
        WN_STRINGERIZE(IsDummy<T1>::val == 0 && IsDummy<T2>::val == 1));
    return (OutCaster<R>::ConvertOut(
        reinterpret_cast<typename OutCaster<R>::InType (*)(
            typename InCaster<T1>::OutType)>(callPt)(
            InCaster<T1>::ConvertOut(t1))));
  }
  R operator()(const T1& t1, const T2& t2) {
    static_assert(IsDummy<T2>::val == 0 && IsDummy<T3>::val == 1,
        WN_STRINGERIZE(IsDummy<T2>::val == 0 && IsDummy<T3>::val == 1));
    return (OutCaster<R>::ConvertOut(
        reinterpret_cast<typename OutCaster<R>::InType (*)(
            typename InCaster<T1>::OutType, typename InCaster<T2>::OutType)>(
            callPt)(
            InCaster<T1>::ConvertOut(t1), InCaster<T2>::ConvertOut(t2))));
  }
  R operator()(const T1& t1, const T2& t2, const T3& t3) {
    static_assert(IsDummy<T3>::val == 0 && IsDummy<T4>::val == 1,
        WN_STRINGERIZE(IsDummy<T3>::val == 0 && IsDummy<T4>::val == 1));
    return (OutCaster<R>::ConvertOut(
        reinterpret_cast<typename OutCaster<R>::InType (*)(
            typename InCaster<T1>::OutType, typename InCaster<T2>::OutType,
            typename InCaster<T3>::OutType)>(callPt)(
            InCaster<T1>::ConvertOut(t1), InCaster<T2>::ConvertOut(t2),
            InCaster<T3>::ConvertOut(t3))));
  }
  R operator()(const T1& t1, const T2& t2, const T3& t3, const T4& t4) {
    static_assert(IsDummy<T4>::val == 0 && IsDummy<T5>::val == 1,
        WN_STRINGERIZE(IsDummy<T4>::val == 0 && IsDummy<T5>::val == 1));
    return (OutCaster<R>::ConvertOut(
        reinterpret_cast<typename OutCaster<R>::InType (*)(
            typename InCaster<T1>::OutType, typename InCaster<T2>::OutType,
            typename InCaster<T3>::OutType, typename InCaster<T4>::OutType)>(
            callPt)(InCaster<T1>::ConvertOut(t1), InCaster<T2>::ConvertOut(t2),
            InCaster<T3>::ConvertOut(t3), InCaster<T4>::ConvertOut(t4))));
  }
  R operator()(
      const T1& t1, const T2& t2, const T3& t3, const T4& t4, const T5& t5) {
    static_assert(IsDummy<T5>::val == 0 && IsDummy<T6>::val == 1,
        WN_STRINGERIZE(IsDummy<T5>::val == 0 && IsDummy<T6>::val == 1));
    return (OutCaster<R>::ConvertOut(
        reinterpret_cast<typename OutCaster<R>::InType (*)(
            typename InCaster<T1>::OutType, typename InCaster<T2>::OutType,
            typename InCaster<T3>::OutType, typename InCaster<T4>::OutType,
            typename InCaster<T5>::OutType)>(callPt)(
            InCaster<T1>::ConvertOut(t1), InCaster<T2>::ConvertOut(t2),
            InCaster<T3>::ConvertOut(t3), InCaster<T4>::ConvertOut(t4),
            InCaster<T5>::ConvertOut(t5))));
  }
  R operator()(const T1& t1, const T2& t2, const T3& t3, const T4& t4,
      const T6& t5, const T6& t6) {
    static_assert(IsDummy<T6>::val == 0, WN_STRINGERIZE(IsDummy<T6>::val == 0));
    return (OutCaster<R>::ConvertOut(
        reinterpret_cast<typename OutCaster<R>::InType (*)(
            typename InCaster<T1>::OutType, typename InCaster<T2>::OutType,
            typename InCaster<T3>::OutType, typename InCaster<T4>::OutType,
            typename InCaster<T5>::OutType, typename InCaster<T6>::OutType)>(
            callPt)(InCaster<T1>::ConvertOut(t1), InCaster<T2>::ConvertOut(t2),
            InCaster<T3>::ConvertOut(t3), InCaster<T4>::ConvertOut(t4),
            InCaster<T5>::ConvertOut(t5), InCaster<T6>::ConvertOut(t6))));
  }
...
public:
  WNMemberFunctionPointer()
    : mReturnType(nullptr), callPt(nullptr), mVirtualIndex(-1) {}
  ...
  R operator()(const T& t) {
    ...
    if (callPt) {
      return (OutCaster<R>::ConvertOut(
          reinterpret_cast<typename OutCaster<R>::InType (*)(
              typename InCaster<T>::OutType)>(callPt)(
              InCaster<T>::ConvertOut(t))));
    } else {
      StructInternalType* m = t.refData;
      void* pt = (*(void***)(m->structLoc))[2 + 2 * mVirtualIndex];
      if (!pt) {
        WNFunctionDefinition* def = reinterpret_cast<WNFunctionDefinition*>(
            (*(void***)(m->structLoc))[2 + 2 * mVirtualIndex + 1]);
        WN_RELEASE_ASSERT(ok ==
                          def->mScriptingEngine->get_function_pointer(
                              def->mContainedFile->mFileName, def->mName,
                              def->mReturn, def->mTypes, pt));
        (*(void***)(m->structLoc))[2 + 2 * mVirtualIndex] = pt;
      }
      return (OutCaster<R>::ConvertOut(
          (reinterpret_cast<typename OutCaster<R>::InType (*)(
                  typename InCaster<T>::OutType)>(pt))(
              InCaster<T>::ConvertOut(t))));
    }
  }
  R operator()(const T& t, const T1& t1) {
    ...
    if (callPt) {
      return (OutCaster<R>::ConvertOut(
          reinterpret_cast<typename OutCaster<R>::InType (*)(
              typename InCaster<T>::OutType, typename InCaster<T1>::OutType)>(
              callPt)(
              InCaster<T>::ConvertOut(t), InCaster<T1>::ConvertOut(t1))));
    } else {
      StructInternalType* m = t.refData;
      void* pt = (*(void***)(m->structLoc))[2 + 2 * mVirtualIndex];
      if (!pt) {
  ...
  R operator()(const T& t, const T1& t1, const T2& t2) {
    ...
    if (callPt) {
      return (OutCaster<R>::ConvertOut(
          reinterpret_cast<typename OutCaster<R>::InType (*)(
              typename InCaster<T>::OutType, typename InCaster<T1>::OutType,
              typename InCaster<T2>::OutType)>(callPt)(
              InCaster<T>::ConvertOut(t), InCaster<T1>::ConvertOut(t1),
              InCaster<T2>::ConvertOut(t2))));
    } else {
      StructInternalType* m = t.refData;
      void* pt = (*(void***)(m->structLoc))[2 + 2 * mVirtualIndex];
      if (!pt) {
      ...
      return (OutCaster<R>::ConvertOut(
          reinterpret_cast<typename OutCaster<R>::InType (*)(
              typename InCaster<T>::OutType, typename InCaster<T1>::OutType,
              typename InCaster<T2>::OutType)>(pt)(InCaster<T>::ConvertOut(t),
              InCaster<T1>::ConvertOut(t1), InCaster<T2>::ConvertOut(t2))));
    }
  }
  R operator()(const T& t, const T1& t1, const T2& t2, const T3& t3) {
    ...
    if (callPt) {
      return (OutCaster<R>::ConvertOut(
          reinterpret_cast<typename OutCaster<R>::InType (*)(
              typename InCaster<T>::OutType, typename InCaster<T1>::OutType,
              typename InCaster<T2>::OutType, typename InCaster<T3>::OutType)>(
              callPt)(InCaster<T>::ConvertOut(t), InCaster<T1>::ConvertOut(t1),
              InCaster<T2>::ConvertOut(t2), InCaster<T3>::ConvertOut(t3))));
    } else {
      StructInternalType* m = t.refData;
      void* pt = (*(void***)(m->structLoc))[2 + 2 * mVirtualIndex];
      if (!pt) {
      ...
      return (OutCaster<R>::ConvertOut(
          reinterpret_cast<typename OutCaster<R>::InType (*)(
              typename InCaster<T>::OutType, typename InCaster<T1>::OutType,
              typename InCaster<T2>::OutType, typename InCaster<T3>::OutType)>(
              pt)(InCaster<T>::ConvertOut(t), InCaster<T1>::ConvertOut(t1),
              InCaster<T2>::ConvertOut(t2), InCaster<T3>::ConvertOut(t3))));
    }
  }
  R operator()(
      const T& t, const T1& t1, const T2& t2, const T3& t3, const T4& t4) {
    static_assert(IsDummy<T4>::val == 0 && IsDummy<T5>::val == 1,
        WN_STRINGERIZE(IsDummy<T4>::val == 0 && IsDummy<T5>::val == 1));
    WN_RELEASE_ASSERT(callPt || mVirtualIndex >= 0);
    if (callPt) {
      return (OutCaster<R>::ConvertOut(
          reinterpret_cast<typename OutCaster<R>::InType (*)(
              typename InCaster<T>::OutType, typename InCaster<T1>::OutType,
              typename InCaster<T2>::OutType, typename InCaster<T3>::OutType,
              typename InCaster<T4>::OutType)>(callPt)(
              InCaster<T>::ConvertOut(t), InCaster<T1>::ConvertOut(t1),
              InCaster<T2>::ConvertOut(t2), InCaster<T3>::ConvertOut(t3),
              InCaster<T4>::ConvertOut(t4))));
    } else {
      StructInternalType* m = t.refData;
      void* pt = (*(void***)(m->structLoc))[2 + 2 * mVirtualIndex];
      if (!pt) {
      ...
      return (OutCaster<R>::ConvertOut(
          reinterpret_cast<typename OutCaster<R>::InType (*)(
              typename InCaster<T>::OutType, typename InCaster<T1>::OutType,
              typename InCaster<T2>::OutType, typename InCaster<T3>::OutType,
              typename InCaster<T4>::OutType)>(pt)(InCaster<T>::ConvertOut(t),
              InCaster<T1>::ConvertOut(t1), InCaster<T2>::ConvertOut(t2),
              InCaster<T3>::ConvertOut(t3), InCaster<T4>::ConvertOut(t4))));
    }
  }
    ...
    if (callPt) {
      return (OutCaster<R>::ConvertOut(
          reinterpret_cast<typename OutCaster<R>::InType (*)(
              typename InCaster<T>::OutType, typename InCaster<T1>::OutType,
              typename InCaster<T2>::OutType, typename InCaster<T3>::OutType,
              typename InCaster<T4>::OutType, typename InCaster<T5>::OutType)>(
              callPt)(InCaster<T>::ConvertOut(t), InCaster<T1>::ConvertOut(t1),
              InCaster<T2>::ConvertOut(t2), InCaster<T3>::ConvertOut(t3),
              InCaster<T4>::ConvertOut(t4), InCaster<T5>::ConvertOut(t5))));
    } else {
      StructInternalType* m = t.refData;
      void* pt = (*(void***)(m->structLoc))[2 + 2 * mVirtualIndex];
      if (!pt) {
  ...
  R operator()(const T& t, const T1& t1, const T2& t2, const T3& t3,
      const T4& t4, const T6& t5, const T6& t6) {
    static_assert(IsDummy<T6>::val == 0, WN_STRINGERIZE(IsDummy<T6>::val == 0));
    WN_RELEASE_ASSERT(callPt || mVirtualIndex >= 0);
    if (callPt) {
      return (OutCaster<R>::ConvertOut(
          reinterpret_cast<typename OutCaster<R>::InType (*)(
              ...
              typename InCaster<T2>::OutType, typename InCaster<T3>::OutType,
              typename InCaster<T4>::OutType, typename InCaster<T5>::OutType,
              typename InCaster<T6>::OutType)>(callPt)(
              InCaster<T>::ConvertOut(t), InCaster<T1>::ConvertOut(t1),
              InCaster<T2>::ConvertOut(t2), InCaster<T3>::ConvertOut(t3),
              InCaster<T4>::ConvertOut(t4), InCaster<T5>::ConvertOut(t5),
              InCaster<T6>::ConvertOut(t6))));
    } else {
      StructInternalType* m = t.refData;
      void* pt = (*(void***)(m->structLoc))[2 + 2 * mVirtualIndex];
      if (!pt) {
      ...
      return (OutCaster<R>::ConvertOut(
          reinterpret_cast<typename OutCaster<R>::InType (*)(
              ...
              typename InCaster<T2>::OutType, typename InCaster<T3>::OutType,
              typename InCaster<T4>::OutType, typename InCaster<T5>::OutType,
              typename InCaster<T6>::OutType)>(pt)(InCaster<T>::ConvertOut(t),
              InCaster<T1>::ConvertOut(t1), InCaster<T2>::ConvertOut(t2),
              InCaster<T3>::ConvertOut(t3), InCaster<T4>::ConvertOut(t4),
              InCaster<T5>::ConvertOut(t5), InCaster<T6>::ConvertOut(t6))));
    }
...
#define DEFINE_TYPED_SCRIPT_CHILD(_name, _parent)                              \
  ...
  typedef WNScriptingObjectRef<ScriptTypePT_##_name*, ScriptType_##_parent>*   \
      ScriptType_##_name##_ref;                                                \
  typedef WNScriptingObject<ScriptTypePT_##_name*, ScriptType_##_parent>       \
      ScriptType_##_name;                                                      \
  template <>                                                                  \
...
#define DEFINE_TYPED_SCRIPT_OBJECT(_name)                                      \
  namespace WNScripting {                                                      \
  struct ScriptTypePT_##_name;                                                 \
  typedef WNScriptingObjectRef<ScriptTypePT_##_name>*                          \
      ScriptType_##_name##_ref;                                                \
...
#define DEFINE_CPP_TYPE(_type)                                                 \
  ...
  struct OutCaster<ScriptPointer_##_type> {                                    \
    ...
    static ScriptPointer_##_type ConvertOut(void* tVal) {                      \
      return (                                                                 \
          ScriptPointer_##_type(reinterpret_cast<StructInternalType*>(tVal))); \
    }                                                                          \
  };                                                                           \
  }
...
#define DEFINE_CPP_TYPE_CHILD(_type, _parent)                                  \
  namespace WNScripting {                                                      \
  typedef ScriptPointerRef<_type, ScriptPointer_##_parent>                     \
      ScriptPointer_##_type##_ref;                                             \
