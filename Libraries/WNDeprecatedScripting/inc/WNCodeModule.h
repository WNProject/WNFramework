...
enum ModuleFlags { eOptimized = 1 << 0, eGenerateDebugData = 1 << 1 };
class WNScriptingEngine;
class WNCodeModule {
public:
  WNCodeModule(WNTypeManager& _typeManager,
      WNScopedVariableList& _scopedVariableList,
      WNScriptingEngine* _scriptingEngine);
  eWNTypeError initialize(uint32_t flags);
  llvm::ExecutionEngine* GetExecutionEngine();
  llvm::Module* GetModule();
  llvm::IRBuilderBase* GetBuilder();
  WNFunctionDefinition* GetFunctionDefinition(
      const char* _name, const std::vector<WNParameter>& _params);
  WNFunctionDefinition* GetFunctionDefinition(
      const char* _name, const std::vector<WNScriptType>& _params);
  eWNTypeError GetCastableFunctionDefinition(const char* _name,
      const std::vector<WNScriptType>& _params,
      WNFunctionDefinition*& _definition);
  eWNTypeError GetCastableVirtualFunction(const char* _name,
      const WNScriptType thisType, const std::vector<FunctionParam>& _params,
      int32_t& _outIndex);
  eWNTypeError GetNonCastableVirtualFunction(const char* _name,
      const WNScriptType thisType, const std::vector<WNScriptType>& _params,
      int32_t& _outIndex);
  eWNTypeError AddFunctionDefinition(const char* _name,
      ...
      WNScriptType _thisType = nullptr, bool _virtual = false);
  eWNTypeError GenerateFunctionDefinition(const char* _name,
      const std::vector<WNScriptType>& _params, const WNScriptType& _return,
      WNFunctionDefinition*& _outFunctionDefinition,
      WNScriptType _thisType = nullptr, bool _virtual = false);
  eWNTypeError AddExternalDefinition(const char* _name, const char* _tag,
      const WNScriptType _thisType, const std::vector<WNScriptType>& _params,
      const WNScriptType& _return);
  eWNTypeError AddExternalScriptFunction(WNFunctionDefinition* functionDef,
      WNFunctionDefinition*& _equivalentFunction);
  eWNTypeError GetThisCallFunction(const char* _name,
      const WNScriptType thisType, const std::vector<FunctionParam>& _params,
