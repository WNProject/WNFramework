...
public:
  WNScriptVariable(WNScriptType _type, const char* _name, llvm::Value* _alloca,
      bool _dontClean = false)
    : mType(_type), mLlvmAlloca(_alloca), mDontClean(_dontClean) {
  ...
  const WNScriptType GetType() const {
    return (mType);
  }
  const char* GetName() const {
    return (mName);
  }
  llvm::Value* GetLocation() const {
    return (mLlvmAlloca);
  }
  bool Clean() const {
    return (!mDontClean);
...
public:
  ...
  virtual eWNTypeError PopScopeBlock(WNCodeModule& _module, bool _cleanStack,
      const WNFunctionDefinition* _def, WNLogging::WNLog& _compilationLog) = 0;
